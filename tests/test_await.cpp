...
TEST_CASE("fl::await - Basic Operations") {
    SUBCASE("await resolved promise returns value") {
    ...
    SUBCASE("await rejected promise returns error") {
    ...
    SUBCASE("await invalid promise returns error") {
        fl::promise<int> invalid_promise; // Default constructor creates invalid promise
        auto result = fl::await(invalid_promise);  // Type automatically deduced!
    ...
    SUBCASE("explicit template parameter still works") {
        auto promise = fl::promise<int>::resolve(42);
        auto result = fl::await<int>(promise);  // Explicit template parameter
...
TEST_CASE("fl::await - Asynchronous Completion") {
    SUBCASE("await waits for promise to be resolved") {
        ...
        // For testing, we'll complete it immediately after starting await
    ...
    SUBCASE("await waits for promise to be rejected") {
        auto promise = fl::promise<int>::create();
        bool promise_completed = false;
...
TEST_CASE("fl::await - Different Value Types") {
    SUBCASE("await with string type") {
    ...
    SUBCASE("await with custom struct") {
        struct TestData {
            int x;
            fl::string name;
...
TEST_CASE("fl::await - Error Handling") {
    SUBCASE("await preserves error message") {
    ...
    SUBCASE("await with custom error") {
...
TEST_CASE("fl::await - Multiple Awaits") {
    SUBCASE("multiple awaits on different promises") {
        ...
        auto result1 = fl::await(promise1);  // Type automatically deduced!
        auto result2 = fl::await(promise2);  // Type automatically deduced!
        auto result3 = fl::await(promise3);  // Type automatically deduced!
    ...
    SUBCASE("await same promise multiple times") {
        auto promise = fl::promise<int>::resolve(999);
        
        auto result1 = fl::await(promise);  // Type automatically deduced!
        auto result2 = fl::await(promise);  // Type automatically deduced!
...
TEST_CASE("fl::await - Boolean Conversion and Convenience") {
    SUBCASE("boolean conversion operator") {
        auto success_promise = fl::promise<int>::resolve(42);
        auto success_result = fl::await(success_promise);
        
        auto error_promise = fl::promise<int>::reject(Error("Error"));
        auto error_result = fl::await(error_promise);
