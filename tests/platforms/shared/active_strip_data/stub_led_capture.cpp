...
//
// NOTE: Each test is a separate FL_TEST_CASE (not subcases) because
// CLEDController's linked list persists across subcases, causing
// stack-use-after-scope when a subcase's local leds[] goes out of scope
// but the controller still references it.
...
FL_TEST_CASE("ClocklessController - LED capture updates on each show") {
...
FL_TEST_CASE("ClocklessController - Multiple strips captured independently") {
