...
TEST_CASE("rmt5_nibble_lut_correctness") {
    ...
    for (int nibble = 0; nibble < 16; nibble++) {
        for (int bit_pos = 0; bit_pos < 4; bit_pos++) {
            ...
            FL_CHECK(lut[nibble][bit_pos].val == expected);
        }
    }
    ...
    FL_CHECK(lut[high_nibble][0].val == zero.val);  // bit 7 = 0
    FL_CHECK(lut[high_nibble][1].val == one.val);   // bit 6 = 1
    FL_CHECK(lut[high_nibble][2].val == one.val);   // bit 5 = 1
    FL_CHECK(lut[high_nibble][3].val == zero.val);  // bit 4 = 0
    FL_CHECK(lut[low_nibble][0].val == one.val);    // bit 3 = 1
    FL_CHECK(lut[low_nibble][1].val == zero.val);   // bit 2 = 0
