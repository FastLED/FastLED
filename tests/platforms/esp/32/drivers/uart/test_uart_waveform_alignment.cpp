...
TEST_CASE("UART Waveform Alignment - Pattern 0b00") {
    ...
    FL_REQUIRE(pattern_00 == 0x11);  // Expected rotated pattern
    ...
    FL_REQUIRE(waveform.size() == 10);  // 1 byte = 10 bits (start + 8 data + stop)
    ...
    // Verify UART framing
    FL_REQUIRE(waveform[0] == false);  // START bit must be LOW
    FL_REQUIRE(waveform[9] == true);   // STOP bit must be HIGH

    // Verify data bits (0x11 = 0b00010001 transmitted LSB-first)
    // Expected: 1-0-0-0-1-0-0-0 (reading from bit 0 to bit 7)
    FL_REQUIRE(waveform[1] == true);   // Bit 0 (LSB) = 1
    FL_REQUIRE(waveform[2] == false);  // Bit 1 = 0
    FL_REQUIRE(waveform[3] == false);  // Bit 2 = 0
    FL_REQUIRE(waveform[4] == false);  // Bit 3 = 0
    FL_REQUIRE(waveform[5] == true);   // Bit 4 = 1
    FL_REQUIRE(waveform[6] == false);  // Bit 5 = 0
    FL_REQUIRE(waveform[7] == false);  // Bit 6 = 0
    FL_REQUIRE(waveform[8] == false);  // Bit 7 (MSB) = 0
...
TEST_CASE("UART Waveform Alignment - All patterns") {
    ...
    SUBCASE("Pattern 0b00 → 0x11") {
        ...
        FL_REQUIRE(waveform[1] == true);   // Bit 0 = 1
        FL_REQUIRE(waveform[2] == false);  // Bit 1 = 0
        FL_REQUIRE(waveform[3] == false);  // Bit 2 = 0
        FL_REQUIRE(waveform[4] == false);  // Bit 3 = 0
        FL_REQUIRE(waveform[5] == true);   // Bit 4 = 1
        FL_REQUIRE(waveform[6] == false);  // Bit 5 = 0
        FL_REQUIRE(waveform[7] == false);  // Bit 6 = 0
        FL_REQUIRE(waveform[8] == false);  // Bit 7 = 0
    ...
    SUBCASE("Pattern 0b01 → 0x19") {
        ...
        FL_REQUIRE(waveform[2] == false);  // Bit 1 = 0
        FL_REQUIRE(waveform[3] == false);  // Bit 2 = 0
        FL_REQUIRE(waveform[4] == true);   // Bit 3 = 1
        FL_REQUIRE(waveform[5] == true);   // Bit 4 = 1
        FL_REQUIRE(waveform[6] == false);  // Bit 5 = 0
        FL_REQUIRE(waveform[7] == false);  // Bit 6 = 0
        FL_REQUIRE(waveform[8] == false);  // Bit 7 = 0
    ...
    SUBCASE("Pattern 0b10 → 0x91") {
        ...
        FL_REQUIRE(waveform[6] == false);  // Bit 5 = 0
        FL_REQUIRE(waveform[7] == false);  // Bit 6 = 0
        FL_REQUIRE(waveform[8] == true);   // Bit 7 = 1
        FL_REQUIRE(waveform[9] == true);   // STOP
    }
...
TEST_CASE("UART Waveform - Original patterns (pre-rotation)") {
    ...
    SUBCASE("Original 0x88 (before rotation)") {
        ...
        FL_REQUIRE(waveform[0] == false);  // START
        FL_REQUIRE(waveform[1] == false);  // Bit 0 = 0  ← Problem: START + B0 + B1 + B2 = 4 consecutive LOWs
        FL_REQUIRE(waveform[2] == false);  // Bit 1 = 0
        FL_REQUIRE(waveform[3] == false);  // Bit 2 = 0
        FL_REQUIRE(waveform[4] == true);   // Bit 3 = 1
        FL_REQUIRE(waveform[5] == false);  // Bit 4 = 0
        FL_REQUIRE(waveform[6] == false);  // Bit 5 = 0
        FL_REQUIRE(waveform[7] == false);  // Bit 6 = 0
        FL_REQUIRE(waveform[8] == true);   // Bit 7 = 1
        FL_REQUIRE(waveform[9] == true);   // STOP
    ...
    SUBCASE("Rotated 0x11 (after rotation)") {
        ...
        FL_REQUIRE(waveform[0] == false);  // START
        FL_REQUIRE(waveform[1] == true);   // Bit 0 = 1  ← Fixed: START(0) followed by HIGH(1) creates clear edge
        FL_REQUIRE(waveform[2] == false);  // Bit 1 = 0
        FL_REQUIRE(waveform[3] == false);  // Bit 2 = 0
        FL_REQUIRE(waveform[4] == false);  // Bit 3 = 0
        FL_REQUIRE(waveform[5] == true);   // Bit 4 = 1
