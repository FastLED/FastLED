...
TEST_CASE("Wave8 UART Encoder - Byte encoding") {
    SUBCASE("Encode byte 0x00 (all bits 0)") {
        ...
        FL_REQUIRE(output[0] == 0x11);  // Bits 7-6: 0b00
        FL_REQUIRE(output[1] == 0x11);  // Bits 5-4: 0b00
        FL_REQUIRE(output[2] == 0x11);  // Bits 3-2: 0b00
    ...
    SUBCASE("Encode byte 0xFF (all bits 1)") {
        ...
        FL_REQUIRE(output[0] == 0x99);  // Bits 7-6: 0b11
        FL_REQUIRE(output[1] == 0x99);  // Bits 5-4: 0b11
        FL_REQUIRE(output[2] == 0x99);  // Bits 3-2: 0b11
    ...
    SUBCASE("Encode byte 0xAA (alternating 1010 1010)") {
        ...
        FL_REQUIRE(output[0] == 0x91);  // Bits 7-6: 0b10
        FL_REQUIRE(output[1] == 0x91);  // Bits 5-4: 0b10
        FL_REQUIRE(output[2] == 0x91);  // Bits 3-2: 0b10
    ...
    SUBCASE("Encode byte 0x55 (alternating 0101 0101)") {
        ...
        FL_REQUIRE(output[0] == 0x19);  // Bits 7-6: 0b01
        FL_REQUIRE(output[1] == 0x19);  // Bits 5-4: 0b01
        FL_REQUIRE(output[2] == 0x19);  // Bits 3-2: 0b01
...
TEST_CASE("Wave8 UART Encoder - Buffer encoding") {
    SUBCASE("Encode single byte") {
        ...
        // 0x42 = 0b01 00 00 10
        FL_REQUIRE(output[0] == 0x19);  // 0b01 → 0x19
        FL_REQUIRE(output[1] == 0x11);  // 0b00 → 0x11
        FL_REQUIRE(output[2] == 0x11);  // 0b00 → 0x11
    ...
    SUBCASE("Encode multiple bytes") {
        ...
        FL_REQUIRE(output[0] == 0x11);
        FL_REQUIRE(output[1] == 0x11);
        FL_REQUIRE(output[2] == 0x11);
        FL_REQUIRE(output[3] == 0x11);

        // Byte 1: 0xFF → 0x99 0x99 0x99 0x99
        FL_REQUIRE(output[4] == 0x99);
        FL_REQUIRE(output[5] == 0x99);
        FL_REQUIRE(output[6] == 0x99);
        FL_REQUIRE(output[7] == 0x99);

        // Byte 2: 0xAA → 0x91 0x91 0x91 0x91
        FL_REQUIRE(output[8] == 0x91);
        FL_REQUIRE(output[9] == 0x91);
        FL_REQUIRE(output[10] == 0x91);
    ...
    SUBCASE("Encode RGB LED (3 bytes)") {
        ...
        FL_REQUIRE(output[0] == 0x99);
        FL_REQUIRE(output[1] == 0x99);
        FL_REQUIRE(output[2] == 0x99);
        FL_REQUIRE(output[3] == 0x99);

        // G=0x00 → 0x11 0x11 0x11 0x11
        FL_REQUIRE(output[4] == 0x11);
        FL_REQUIRE(output[5] == 0x11);
        FL_REQUIRE(output[6] == 0x11);
        FL_REQUIRE(output[7] == 0x11);

        // B=0x00 → 0x11 0x11 0x11 0x11
        FL_REQUIRE(output[8] == 0x11);
        FL_REQUIRE(output[9] == 0x11);
        FL_REQUIRE(output[10] == 0x11);
    ...
    SUBCASE("Encode 100 RGB LEDs") {
        ...
        FL_REQUIRE(output[0] == 0x11);   // R byte 0 (index 0: even → 0x00)
        FL_REQUIRE(output[4] == 0x99);   // G byte 0 (index 1: odd → 0xFF)
        FL_REQUIRE(output[8] == 0x11);   // B byte 0 (index 2: even → 0x00)
        ...
        FL_REQUIRE(output[last_led_offset + 0] == 0x99);  // R=0xFF → 0x99
        FL_REQUIRE(output[last_led_offset + 4] == 0x11);  // G=0x00 → 0x11
        FL_REQUIRE(output[last_led_offset + 8] == 0x99);  // B=0xFF → 0x99
    }
}
...
TEST_CASE("Wave8 UART Encoder - Bit rotation correctness") {
    ...
    SUBCASE("Rotation preserves bit count (HIGH bits remain same)") {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            FL_REQUIRE(original_count == rotated_count);

            // Verify current LUT uses rotated pattern
            FL_REQUIRE(detail::encodeUart2Bits(i) == rotated_patterns[i]);
        }
    }
