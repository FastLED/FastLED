...
TEST_CASE("SlabAllocator - Contiguous memory within slab") {
    ...
    SUBCASE("First 8 allocations should be contiguous") {
        ...
        for (size_t i = 1; i < ptrs.size(); ++i) {
            ...
            CHECK_EQ(diff, expected_block_size);
        }
        ...
        CHECK_EQ(total_range, expected_range);
    ...
    SUBCASE("Memory boundaries verification") {
        ...
        CHECK_EQ(actual_range, slab_size);
        ...
        for (TestObject* ptr : ptrs) {
            ...
            CHECK_EQ(offset_from_start % block_size, 0);
        }
...
TEST_CASE("SlabAllocator - Multiple slabs behavior") {
    ...
    SUBCASE("Allocation across multiple slabs") {
        ...
        CHECK_EQ(allocator.getSlabCount(), 3);  // Should have created 3 slabs
        CHECK_EQ(allocator.getTotalAllocated(), total_allocs);
        ...
        for (size_t i = 0; i < ptrs.size(); ++i) {
            ...
            CHECK_EQ(addr % alignof(TestObject), 0);
        ...
        for (size_t i = 0; i < ptrs.size(); ++i) {
            CHECK_EQ(ptrs[i]->data[0], static_cast<int>(i + 100));
            CHECK_EQ(ptrs[i]->data[1], static_cast<int>(i + 200));
            CHECK_EQ(ptrs[i]->data[2], static_cast<int>(i + 300));
            CHECK_EQ(ptrs[i]->data[3], static_cast<int>(i + 400));
        }
...
TEST_CASE("SlabAllocator - Memory layout verification") {
    ...
    SUBCASE("Detailed memory layout check") {
        ...
        for (size_t i = 0; i < ptrs.size(); ++i) {
            ...
            CHECK_EQ(actual_addr, expected_addr);
        }
        ...
        CHECK_EQ(total_span, expected_span);
...
TEST_CASE("SlabAllocator - Edge cases") {
    ...
    SUBCASE("Allocation and deallocation pattern") {
        ...
        CHECK_EQ(allocator.getSlabCount(), 1);  // Still only one slab
    ...
    SUBCASE("Bulk allocation fallback") {
        ...
        CHECK_EQ(allocator.getTotalAllocated(), 0);  // Slab stats unchanged
        CHECK_EQ(allocator.getSlabCount(), 0);       // No slabs created
...
TEST_CASE("SlabAllocator - STL compatibility") {
    ...
    SUBCASE("Allocator equality") {
        ...
        CHECK_EQ(alloc1, alloc2);  // All instances should be equivalent
...
TEST_CASE("allocator_inlined_slab - Basic functionality") {
    ...
    SUBCASE("Single allocation within inlined capacity") {
        ...
        int total_size = allocator.total_size();
        CHECK_EQ(total_size, 1);
        CHECK_EQ(allocator.inlined_capacity(), 4);
        ...
        *ptr = 42;
        ...
        CHECK_EQ(allocator.total_size(), 0);
    }
    ...
    SUBCASE("Multiple allocations within inlined capacity") {
        ...
        CHECK(allocator.is_using_inlined());
        CHECK_EQ(allocator.total_size(), num_allocs);
        
        // Verify all data is intact
        for (size_t i = 0; i < ptrs.size(); ++i) {
            auto curr_int = *ptrs[i];
            auto exp_int = static_cast<int>(i + 100);
            CHECK_EQ(curr_int, exp_int);
        }
    ...
    SUBCASE("Allocation beyond inlined capacity") {
        ...
        // Verify all data is intact
        for (size_t i = 0; i < ptrs.size(); ++i) {
            CHECK_EQ(*ptrs[i], static_cast<int>(i + 200));
        }
...
TEST_CASE("allocator_inlined_slab - Memory layout and contiguity") {
    ...
    SUBCASE("Inlined memory layout verification") {
        ...
        for (size_t i = 1; i < ptrs.size(); ++i) {
            ...
            CHECK_EQ(diff, sizeof(TestObject));
        }
    ...
    SUBCASE("Mixed inlined and heap allocation layout") {
        ...
        inlined_ptrs.clear();
        heap_ptrs.clear();
        inlined_ptrs.swap(fl::vector<TestObject*>());
        heap_ptrs.swap(fl::vector<TestObject*>());
    }
}
...
TEST_CASE("allocator_inlined_slab - Bulk allocation behavior") {
    ...
    SUBCASE("Mixed single and bulk allocations") {
        ...
        for (size_t i = 0; i < single_ptrs.size(); ++i) {
            CHECK_EQ(*single_ptrs[i], static_cast<int>(i + 10));
        }
        
        for (int i = 0; i < 4; ++i) {
            CHECK_EQ(bulk_ptr[i], i + 1000);
        }
...
TEST_CASE("allocator_inlined_slab - Free slot management") {
    ...
    SUBCASE("Reuse of freed inlined slots") {
        ...
        CHECK(allocator.is_using_inlined());
        CHECK_EQ(allocator.total_size(), 4);
    ...
    SUBCASE("Complex allocation/deallocation pattern") {
        ...
        CHECK_EQ(*new_ptrs[0], 1000);
        CHECK_EQ(*new_ptrs[1], 1001);
        CHECK_EQ(*new_ptrs[2], 1002);
...
TEST_CASE("allocator_inlined_slab - Copy and assignment") {
    ...
    SUBCASE("Copy constructor with inlined data") {
        ...
        CHECK_EQ(allocator2.total_size(), allocator1.total_size());
        CHECK_EQ(allocator2.is_using_inlined(), allocator1.is_using_inlined());
...
TEST_CASE("allocator_inlined_slab - Clear functionality") {
    ...
    SUBCASE("Clear inlined allocations") {
        ...
        auto total_size = allocator.total_size();
        CHECK_EQ(total_size, 3);
        CHECK(allocator.is_using_inlined());
    ...
    SUBCASE("Clear mixed allocations") {
        ...
        CHECK_EQ(allocator.total_size(), 6);
        CHECK(!allocator.is_using_inlined());
...
TEST_CASE("allocator_inlined_slab - Edge cases and error handling") {
    ...
    SUBCASE("Large bulk allocation") {
        ...
        // Verify data integrity
        for (size_t i = 0; i < large_count; ++i) {
            CHECK_EQ(bulk_ptr[i], static_cast<int>(i + 1000));
        }
    ...
    SUBCASE("Stress test with many allocations") {
        ...
        fl::vector_inlined<int*, 100> ptrs;
        ...
        for (size_t i = 0; i < ptrs.size(); ++i) {
            CHECK_EQ(*ptrs[i], static_cast<int>(i));
        }
...
TEST_CASE("allocator_inlined_slab - Performance characteristics") {
    ...
    SUBCASE("Allocation speed comparison") {
        ...
        CHECK_EQ(ptrs.size(), test_count);
        
        // Verify data integrity
        for (size_t i = 0; i < ptrs.size(); ++i) {
            auto curr_int = *ptrs[i];
            auto expected_int = static_cast<int>(i);
            CHECK_EQ(curr_int, expected_int);
        }
...
TEST_CASE("allocator_inlined_slab - Complex object types") {
    ...
    SUBCASE("Complex object allocation") {
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            for (int j = 0; j < 4; ++j) {
                CHECK_EQ(ptr->data[j], j);
            }
            
            ptrs.push_back(ptr);
        }
        
        CHECK_EQ(allocator.total_size(), 3);
        ...
        CHECK_EQ(ptrs[0]->str, "test1");
        CHECK_EQ(ptrs[1]->str, "test2");
        CHECK_EQ(ptrs[2]->str, "test3");
    ...
    SUBCASE("Complex object with heap transition") {
        ...
        CHECK_EQ(allocator.total_size(), 5);
        
        // Verify all objects are intact
        for (size_t i = 0; i < ptrs.size(); ++i) {
            fl::string expected_name("obj");
            expected_name.append(fl::to_string(i));
            CHECK_EQ(ptrs[i]->str, expected_name);
            CHECK_EQ(ptrs[i]->vec.size(), 2);
            CHECK_EQ(ptrs[i]->vec[0], static_cast<int>(i));
            CHECK_EQ(ptrs[i]->vec[1], static_cast<int>(i + 10));
        }
