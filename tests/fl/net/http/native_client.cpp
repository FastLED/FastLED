...
// Port for real socket I/O tests
static const int kClientSocketPort = 58911;
...
FL_TEST_CASE("NativeHttpClient - Connection state transitions with server") {
    NativeHttpServer server(kTestPort1);
...
FL_TEST_CASE("NativeHttpClient - Multiple instances with server") {
    NativeHttpServer server1(kTestPort1);
    NativeHttpServer server2(kTestPort2);
...
// =============================================================================
// Real socket I/O tests - consolidated into subcases sharing one server
// =============================================================================

FL_TEST_CASE("NativeHttpClient - Socket I/O") {
    NativeHttpServer server(kClientSocketPort);
    server.setNonBlocking(true);
    FL_REQUIRE(server.start());

    FL_SUBCASE("Connect to live server succeeds") {
        NativeHttpClient client("localhost", kClientSocketPort);
        FL_CHECK(client.connect());
    ...
    FL_SUBCASE("Client sends data to server") {
        NativeHttpClient client("localhost", kClientSocketPort);
        client.setNonBlocking(true);
    ...
    FL_SUBCASE("Client receives data from server") {
        NativeHttpClient client("localhost", kClientSocketPort);
        client.setNonBlocking(true);
        FL_REQUIRE(client.connect());
        FL_REQUIRE(pollUntil([&]() {
    ...
    FL_SUBCASE("Large payload transfer") {
        NativeHttpClient client("localhost", kClientSocketPort);
        client.setNonBlocking(true);
        FL_REQUIRE(client.connect());
        FL_REQUIRE(pollUntil([&]() {
