...
TEST_CASE("AsyncLogger registry starts empty") {
    ...
    FL_REQUIRE(after_second_access == after_first_access);

    // Verify we got the same logger instance
    FL_REQUIRE(&logger1 == &logger2);
}
...
TEST_CASE("AsyncLogger registry grows on first use") {
    ...
    FL_REQUIRE(after_access >= initial_size);
    FL_REQUIRE(after_access <= initial_size + 1);
}
...
TEST_CASE("AsyncLogger multiple different loggers increase registry size") {
    ...
    FL_REQUIRE(after_logger1 >= initial_size);
    FL_REQUIRE(after_logger2 >= after_logger1);
    FL_REQUIRE(after_logger3 >= after_logger2);

    // Verify we got different logger instances
    FL_REQUIRE(&logger1 != &logger2);
    FL_REQUIRE(&logger2 != &logger3);
    FL_REQUIRE(&logger1 != &logger3);
}
...
TEST_CASE("AsyncLogger basic push/flush functionality") {
    ...
    // Initially empty
    FL_REQUIRE(logger.empty());
    FL_REQUIRE(logger.size() == 0);
