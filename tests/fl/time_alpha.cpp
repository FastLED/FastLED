...
TEST_CASE("Test transition ramp") {
    ...
    FL_REQUIRE(ramp.update8(t0) == 0);

    // mid‑rise: 5 ms → (5*255/10) ≈ 127
    FL_REQUIRE(ramp.update8(t0 + 5) == static_cast<uint8_t>((5 * 255) / 10));

    // end of rise: 10 ms → full on
    FL_REQUIRE(ramp.update8(t0 + 10) == 255);

    // plateau: well within [10, 90) ms
    FL_REQUIRE(ramp.update8(t0 + 50) == 255);
    ...
    FL_REQUIRE_EQ(expected, value);

    // after latch: 110 ms → off
    // FL_REQUIRE(ramp.update8(t0 + 110) == 0);

    // now do it again
    ramp.trigger(200);
    // at start: still at zero
    FL_REQUIRE(ramp.update8(200) == 0);
    // mid‑rise: 205 ms → (5*255/10) ≈ 127
    FL_REQUIRE(ramp.update8(205) == static_cast<uint8_t>((5 * 255) / 10));
    // end of rise: 210 ms → full on
    FL_REQUIRE(ramp.update8(210) == 255);
    // plateau: well within [210, 290) ms
    FL_REQUIRE(ramp.update8(250) == 255);
    // mid‑fall: elapsed=295 ms → fallingElapsed=5 ms → 255 - (5*255/10) ≈ 128
    FL_REQUIRE(ramp.update8(315) == static_cast<uint8_t>(255 - (5 * 255) / 10));
    // after latch: 310 ms → off
    FL_REQUIRE(ramp.update8(320) == 0);
...
TEST_CASE("Real world Bug") {
    ...
    FL_REQUIRE_EQ(value, 0);

    value = transition.update8(6900 + 500);
    FL_REQUIRE_EQ(value, 255);

    value = transition.update8(6900 + 250);
    FL_REQUIRE_EQ(value, 127);
}