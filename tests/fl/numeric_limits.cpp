...
TEST_CASE("fl::numeric_limits<long>") {
    ...
    SUBCASE("consistent with sizeof") {
        ...
        if (sizeof(long) == 4) {
            ...
            FL_CHECK_EQ(numeric_limits<long>::min(), -2147483647L - 1);
        }
        // On systems where long is 8 bytes
        else if (sizeof(long) == 8) {
            FL_CHECK_EQ(numeric_limits<long>::max(), 9223372036854775807L);
            FL_CHECK_EQ(numeric_limits<long>::min(), -9223372036854775807L - 1);
        }
    }
}
...
TEST_CASE("fl::numeric_limits<unsigned long>") {
    ...
    SUBCASE("consistent with sizeof") {
        if (sizeof(unsigned long) == 4) {
            ...
            FL_CHECK_EQ(numeric_limits<unsigned long>::max(), 18446744073709551615ul);
        }
    }
}
...
TEST_CASE("fl::numeric_limits compile-time evaluation") {
    SUBCASE("all limits are constexpr") {
        ...
        FL_CHECK(int_min < int_max);
        FL_CHECK(uint_max > 0);
        FL_CHECK(long_min < long_max);
        FL_CHECK(ll_min < ll_max);
        FL_CHECK(float_min < float_max);
        FL_CHECK(double_min < double_max);
    }
}
...
TEST_CASE("fl::numeric_limits integral relationships") {
    ...
    SUBCASE("signed min magnitude equals signed max + 1") {
        ...
        FL_CHECK_EQ(int_min_abs, int_max + 1);
    }
}
