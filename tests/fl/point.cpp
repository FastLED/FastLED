...
#define FL_REQUIRE_APPROX(a, b, tolerance) \
    do { \
        if (fl::fabs((a - b)) > (tolerance)) { \
            fl::cout << "REQUIRE_APPROX failed: " << #a << " = " << (a) \
                      << ", " << #b << " = " << (b) \
                      << ", tolerance = " << (tolerance) << fl::endl; \
            fl::exit(1); \
        } \
    } while (0)
...
TEST_CASE("0 is 0 distance from diagonal line through the center") {
    ...
    FL_REQUIRE_APPROX(projected.x, 0.0f, 0.001f);
    FL_REQUIRE_APPROX(projected.y, 0.0f, 0.001f);
    FL_REQUIRE_APPROX(dist, 0.0f, 0.001f);
}
...
TEST_CASE("point closest to line") {
    ...
    FL_REQUIRE_APPROX(projected.x, 25.0f, 0.001f);
    FL_REQUIRE_APPROX(projected.y, 25.0f, 0.001f);

    // Distance should be sqrt((50-25)^2 + (0-25)^2) = sqrt(1250) ≈ 35.355
    FL_REQUIRE_APPROX(dist, 35.355f, 0.001f);
}
