...
TEST_CASE("Simple JsonConsole test") {
    ...
    fl::JsonConsolePtr console = fl::make_shared<fl::JsonConsole>(mockAvailable, mockRead, mockWrite);
...
TEST_CASE("JsonConsole polling system test") {
    ...
    auto console = fl::make_shared<fl::JsonConsole>(mockAvailable, mockRead, mockWrite);
    console->init();
    ...
    fl::Json componentArray = fl::Json::createArray();
    componentArray.add(componentObj);
    
    fl::string componentJson = componentArray.serialize();
