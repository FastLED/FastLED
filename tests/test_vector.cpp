...
TEST_CASE("Container alignment requirements - systematic 8-byte alignment") {
    // This test verifies our systematic approach to 8-byte alignment across all container classes
    // to prevent WASM runtime errors like "reference binding to misaligned address"
    //
    // We've standardized on alignas(8) for all major container classes:
    // - InlinedMemoryBlock, FixedVector, InlinedVector, HeapVector, SortedHeapVector, HashMap
    
    // Test InlinedMemoryBlock alignment (foundation for other containers)
    {
        using TestBlock = fl::InlinedMemoryBlock<double, 10>;
        REQUIRE_GE(alignof(TestBlock), 8);
        
        TestBlock block1, block2;
        REQUIRE_EQ(reinterpret_cast<uintptr_t>(&block1) % 8, 0);
        REQUIRE_EQ(reinterpret_cast<uintptr_t>(&block2) % 8, 0);
    }
    
    // Test FixedVector alignment 
    {
        using TestVector = fl::FixedVector<fl::function<void()>, 5>;
        REQUIRE_GE(alignof(TestVector), 8);
        
        TestVector vec1, vec2;
        REQUIRE_EQ(reinterpret_cast<uintptr_t>(&vec1) % 8, 0);
        REQUIRE_EQ(reinterpret_cast<uintptr_t>(&vec2) % 8, 0);
        
        // Test that stored function objects work correctly
        vec1.push_back([]() { /* test lambda */ });
        REQUIRE_EQ(vec1.size(), 1);
    }
    
    // Test InlinedVector alignment
    {
        using TestVector = fl::InlinedVector<fl::function<int(int)>, 3>;
        REQUIRE_GE(alignof(TestVector), 8);
        ...
        // Test that stored function objects work correctly
        vec1.push_back([](int x) { return x * 2; });
        REQUIRE_EQ(vec1.size(), 1);
        REQUIRE_EQ(vec1[0](5), 10);
    }
    
    // Test HeapVector alignment
    {
        using TestVector = fl::HeapVector<fl::Variant<int, double, fl::string>>;
        REQUIRE_GE(alignof(TestVector), 8);
        ...
        // Test that complex stored objects work correctly
        vec1.push_back(42);
        vec1.push_back(3.14159);
        vec1.push_back(fl::string("test"));
        REQUIRE_EQ(vec1.size(), 3);
    }
    
    // Test SortedHeapVector alignment
    {
        using TestVector = fl::SortedHeapVector<fl::function<bool()>>;
        REQUIRE_GE(alignof(TestVector), 8);
    ...
    // Test HashMap alignment (the specific class from the error)
    {
        using TestMap = fl::HashMap<fl::u32, fl::function<void(int)>>;
        REQUIRE_GE(alignof(TestMap), 8);
        
        TestMap map1, map2;
        REQUIRE_EQ(reinterpret_cast<uintptr_t>(&map1) % 8, 0);
        REQUIRE_EQ(reinterpret_cast<uintptr_t>(&map2) % 8, 0);
        
        // Test the exact scenario that was failing - function storage and retrieval
        fl::u32 id = 123;
        map1[id] = [](int x) { 
            // Function that might require alignment
            double result = static_cast<double>(x) * 3.14159;
            (void)result;
        };
        
        REQUIRE_EQ(map1.size(), 1);
        
        // This operation was failing with alignment errors
        auto it = map1.find(id);
        REQUIRE_NE(it, map1.end());
        
        const fl::function<void(int)>& callback = it->second;
        REQUIRE(callback);
        callback(42); // Should not crash with alignment errors
        
        // Test copying the map (triggers Entry copying)
        TestMap map_copy = map1;
        auto it_copy = map_copy.find(id);
        REQUIRE_NE(it_copy, map_copy.end());
        it_copy->second(84); // Should work without alignment errors
    }
    
    // Test arrays of aligned containers
    {
        using TestVector = fl::FixedVector<double, 4>;
        TestVector vectors[10];
        
        // Each vector in the array should be properly aligned
        for (int i = 0; i < 10; ++i) {
            REQUIRE_EQ(reinterpret_cast<uintptr_t>(&vectors[i]) % 8, 0);
            vectors[i].push_back(static_cast<double>(i) * 3.14159);
            REQUIRE_EQ(vectors[i].size(), 1);
        }
    }
    
    // Test heap-allocated aligned containers
    {
        using TestMap = fl::HashMap<int, fl::Variant<fl::string, double>>;
        
        auto* heap_map = new TestMap();
        REQUIRE_EQ(reinterpret_cast<uintptr_t>(heap_map) % 8, 0);
        
        (*heap_map)[1] = fl::string("test");
        (*heap_map)[2] = 3.14159;
        
        REQUIRE_EQ(heap_map->size(), 2);
