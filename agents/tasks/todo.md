...
- User subsequently chose upstream Wild Linux-only because Windows GNU uses special linker/DLL injection and macOS linker behavior is uncertain. The installer now targets the publisher-checksummed Wild 0.10.0 Linux GNU release, and only Linux workflow steps set `FASTLED_NATIVE_LINKER`; macOS and Windows keep existing linking. Linux Wild live and hosted validation are pending.
