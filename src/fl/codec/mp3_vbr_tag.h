...
///
/// Deliberately header-only, with the definitions inline below rather than in a
/// mp3_vbr_tag.cpp.hpp. Splitting them out looks tidier and was tried: the
/// repo's linter hard-bans including a .cpp.hpp from anything but a _build.*
/// file, and this parser has three consumers that cannot go through one --
/// the streaming decoder, the unit tests, and the CI conformance harness, which
/// builds against `-I src` with no FastLED library to link. Inline definitions
/// in the header are what that constraint leaves, and they are ODR-safe.
namespace detail {
