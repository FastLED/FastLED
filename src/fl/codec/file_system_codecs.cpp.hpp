...
FramePtr FileSystem::loadJpeg(const char *path, const JpegConfig &config,
    ...
    if (!file.is_open()) {
        ...
        FL_WARN("Failed to open JPEG file: %s", path); return FramePtr();
    }
    ...
    if (bytesRead != fileSize) {
        ...
        FL_WARN("Failed to read complete JPEG file: %s", path); return FramePtr();
    }
...
fl::Mp3DecoderPtr FileSystem::openMp3(const char *path, fl::string *error_message) {
    ...
    if (!file.is_open()) {
        if (error_message) { *error_message = "Failed to open file: "; error_message->append(path); }
        FL_WARN("Failed to open MP3 file: %s", path); return fl::Mp3DecoderPtr();
    }
    ...
    if (!decoder->begin(file.rdbuf())) {
        ...
        FL_WARN("Failed to initialize MP3 decoder for: %s", path); return fl::Mp3DecoderPtr();
    }
    return decoder;
}
