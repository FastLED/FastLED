/// @file fl/system/file_system_sd.cpp.hpp
...
/// (~15 KB on ESP32-S3 NEOPIXEL Blink) with it.
///
/// See FastLED #2773 item 1.2 for the audit that surfaced this.
...
// the real Arduino `SD`/`FS` libraries. Pulling those into `fl.system.sd+.cpp.o`
// is fine — the linker only links this TU when the user calls beginSd.
