    ...
    if (best_port >= 0 && best_count < num_active) {
        for (u8 i = 0; i < 8; ++i) {
            ...
            if (port_ids[i] != best_port) {
                FL_WARN("digitalMultiWrite8: pin %s (port %s) disabled — not on majority port %s", mPins[i], port_ids[i], best_port);
                mPins[i] = -1;
            }
        }
    }
...
void DigitalMultiWrite16::init(const Pins16& pins) {
    ...
    if (best_port >= 0 && best_count < num_active) {
        for (u8 i = 0; i < 16; ++i) {
            ...
            if (port_ids[i] != best_port) {
                FL_WARN("digitalMultiWrite16: pin %s (port %s) disabled — not on majority port %s", mPins[i], port_ids[i], best_port);
                mPins[i] = -1;
            }
        }
    }
