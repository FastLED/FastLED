...
public:
    ...
    uint32_t count() const noexcept {
        ...
        // For the last block, we need to be careful about counting only valid bits
        if (block_count > 0) {
            block_type last_block = _blocks[block_count - 1];
            // If N is not a multiple of bits_per_block, mask out the unused bits
            if (N % bits_per_block != 0) {
                const uint32_t valid_bits = N % bits_per_block;
                // Create a mask with only the valid bits set to 1
                block_type mask = (valid_bits == bits_per_block) ? ~block_type(0) : ((block_type(1) << valid_bits) - 1);
                last_block &= mask;
            }
...
public:
    ...
    friend BitsetInlined operator&(const BitsetInlined& lhs, const BitsetInlined& rhs) noexcept {
        ...
        if (result._storage.template is<fixed_bitset>() && rhs._storage.template is<fixed_bitset>()) {
            // Both are fixed, use the fixed implementation
            *result._storage.template ptr<fixed_bitset>() &= *rhs._storage.template ptr<fixed_bitset>();
        } else {
            // At least one is dynamic, handle bit by bit
            uint32_t min_size = result.size() < rhs.size() ? result.size() : rhs.size();
    ...
    friend BitsetInlined operator|(const BitsetInlined& lhs, const BitsetInlined& rhs) noexcept {
        ...
        if (result._storage.template is<fixed_bitset>() && rhs._storage.template is<fixed_bitset>()) {
            ...
            *result._storage.template ptr<fixed_bitset>() |= *rhs._storage.template ptr<fixed_bitset>();
            ...
            // At least one is dynamic, handle bit by bit
            uint32_t max_size = result.size() > rhs.size() ? result.size() : rhs.size();
