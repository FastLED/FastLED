...
template<typename T> class Ptr; // Forward declaration for migration compatibility
...
struct ControlBlockBase {
    ...
    // No-tracking aware increment/decrement
    void add_shared_ref() {
...
public:
    ...
    // MIGRATION: Constructor from Ptr for compatibility during migration
    template<typename Y>
    shared_ptr(const fl::Ptr<Y>& ptr) : ptr_(ptr.get()), control_block_(nullptr) {
        if (ptr_) {
            // Create a new control block that doesn't own the object
            // since Ptr has its own reference counting
            control_block_ = new detail::ControlBlock<T, detail::no_op_deleter<T>>(
                ptr_, detail::no_op_deleter<T>{}, false);  // track = false
        }
    }
    ...
    // Enhanced use_count that returns 0 for no-tracking shared_ptrs
...
// NEW: make_shared_no_tracking - Creates a shared_ptr that does not modify the reference count
