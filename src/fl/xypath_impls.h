...
class RosePath : public XYPathGenerator {
  public:
    ...
    // even
    RosePath(const fl::shared_ptr<RosePathParams> &p = fl::make_intrusive<RosePathParams>());
...
class PhyllotaxisPath : public XYPathGenerator {
  public:
    ...
    PhyllotaxisPath(
        const fl::shared_ptr<PhyllotaxisParams> &p = fl::make_intrusive<PhyllotaxisParams>());
    vec2f compute(float alpha) override;
    const string name() const override;
...
class GielisCurvePath : public XYPathGenerator {
  public:
    ...
    GielisCurvePath(
        const fl::shared_ptr<GielisCurveParams> &p = fl::make_intrusive<GielisCurveParams>());
    vec2f compute(float alpha) override;
    const string name() const override;
...
class CatmullRomPath : public XYPathGenerator {
  public:
    CatmullRomPath(const fl::shared_ptr<CatmullRomParams> &p = fl::make_intrusive<CatmullRomParams>());
