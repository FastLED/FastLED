...
class Potentiometer {
  public:
    ...
    void injectTestValue(u16 value) {
        mCurrentValue = value;
        // Manually trigger change detection logic
        u16 diff = (value > mLastValue) ? (value - mLastValue) : (mLastValue - value);
  ...
  private:
    ...
    u16 mCurrentValue = 0;     // Current raw ADC value
    u16 mLastValue = 0;        // Last value that triggered callbacks
    u16 mHysteresis = 0;       // Hysteresis threshold in raw ADC units
    ...
    u16 mMinValue = 0;         // Raw value that maps to 0.0 (default: 0)
    u16 mMaxValue = 0;         // Raw value that maps to 1.0 (default: ADC max)
