...
template <typename T>
void typed_destroy_thunk(void* storage) FL_NO_EXCEPT {
    fl::bit_cast_ptr<T>(storage)->~T();
}

template <typename T>
void typed_copy_thunk(void* dst, const void* src) FL_NO_EXCEPT {
    const T* source = fl::bit_cast_ptr<const T>(src);
    new (dst) T(*source);
}

template <typename T>
void typed_move_thunk(void* dst, void* src) FL_NO_EXCEPT {
...
inline void variant_noop_destroy(void* /*storage*/) FL_NO_EXCEPT {}

template <fl::size N>
void variant_trivial_copy_n(void* dst, const void* src) FL_NO_EXCEPT {
    fl::memcpy(dst, src, N);
}

template <fl::size N>
void variant_trivial_move_n(void* dst, void* src) FL_NO_EXCEPT {
    fl::memcpy(dst, src, N);
}
...
class FL_ALIGN_AS_T(max_align<Types...>::value) variant {
  public:
    ...
    variant() FL_NO_EXCEPT : _tag(Empty) {}

    template <typename T, typename = typename fl::enable_if<
                              contains_type<T, Types...>::value>::type>
    variant(const T &value) FL_NO_EXCEPT : _tag(Empty) {
        construct<T>(value);
    }

    template <typename T, typename = typename fl::enable_if<
                              contains_type<T, Types...>::value>::type>
    variant(T &&value) FL_NO_EXCEPT : _tag(Empty) {
        construct<T>(fl::move(value));
    }

    variant(const variant &other) FL_NO_EXCEPT : _tag(Empty) {
        if (!other.empty()) {
            copy_construct_from(other);
        }
    }

    variant(variant &&other) FL_NO_EXCEPT : _tag(Empty) {
        if (!other.empty()) {
    ...
    variant &operator=(variant &&other) FL_NO_EXCEPT {
        if (this != &other) {
            reset();
            if (!other.empty()) {
                move_construct_from(other);
    ...
    template <typename T, typename = typename fl::enable_if<
                              contains_type<T, Types...>::value>::type>
    variant &operator=(const T &value) FL_NO_EXCEPT {
    ...
    template <typename T, typename = typename fl::enable_if<
                              contains_type<T, Types...>::value>::type>
    variant &operator=(T &&value) FL_NO_EXCEPT {
        reset();
        construct<T>(fl::move(value));
        return *this;
    }
    ...
    emplace(Args &&...args) FL_NO_EXCEPT {
        reset();
    ...
    Tag tag() const FL_NO_EXCEPT { return _tag; }
    bool empty() const FL_NO_EXCEPT { return _tag == Empty; }

    template <typename T> bool is() const FL_NO_EXCEPT {
        return _tag == type_to_tag<T>();
    }

    template <typename T> T *ptr() FL_NO_EXCEPT {
    ...
    template <typename T> const T *ptr() const FL_NO_EXCEPT {
        if (!is<T>()) return nullptr;
        // Use bit_cast_ptr for safe type-punning on properly aligned storage
        // The storage is guaranteed to be properly aligned by alignas(max_align<Types...>::value)
    ...
    template <typename T> T &get() FL_NO_EXCEPT {
    ...
    /// @note Asserts if the variant doesn't contain type T. Use is<T>() to check first.
    /// @warning Will crash if called with wrong type - this is intentional for fast failure
    template <typename T> const T &get() const FL_NO_EXCEPT {
        // Dereference ptr() directly - will crash with null pointer access if wrong type
        // This provides fast failure semantics similar to std::variant
        return *ptr<T>();
    }

    template <typename T> bool equals(const T &other) const FL_NO_EXCEPT {
        if (auto p = ptr<T>()) {
            return *p == other;
        }
        return false;
    ...
    template <typename Visitor> void visit(Visitor &visitor) FL_NO_EXCEPT {
        if (_tag == Empty)
            return;
    ...
    template <typename Visitor> void visit(Visitor &visitor) const FL_NO_EXCEPT {
        if (_tag == Empty)
            return;
  ...
  private:
    ...
    static void visit_fn(void *storage, Visitor &v) FL_NO_EXCEPT {
        // Use bit_cast_ptr for safe type-punning on properly aligned storage
        // The storage is guaranteed to be properly aligned by alignas(max_align<Types...>::value)
    ...
    template <typename T, typename Visitor>
    static void visit_fn_const(const void *storage, Visitor &v) FL_NO_EXCEPT {
        // Use bit_cast_ptr for safe type-punning on properly aligned storage
        // The storage is guaranteed to be properly aligned by alignas(max_align<Types...>::value)
    ...
    void destroy_current() FL_NO_EXCEPT {
    ...
    void copy_construct_from(const variant &other) FL_NO_EXCEPT {
        FL_DISABLE_WARNING_PUSH
        FL_DISABLE_WARNING(array-bounds)
    ...
    void move_construct_from(variant &other) FL_NO_EXCEPT {
        FL_DISABLE_WARNING_PUSH
        FL_DISABLE_WARNING(array-bounds)
    ...
    template <typename T> static constexpr Tag type_to_tag() FL_NO_EXCEPT {
    ...
    template <typename T, typename... Args> void construct(Args &&...args) FL_NO_EXCEPT {
