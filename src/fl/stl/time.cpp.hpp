...
// NOTE: FASTLED_TESTING must be checked BEFORE FASTLED_STUB_IMPL because both can be defined
    ...
    #include <stdint.h>  // okay banned header (platform-specific primitive types)
    ...
    #include <Arduino.h>  // okay banned header (platform-specific Arduino API)
    ...
    #include <Arduino.h>  // okay banned header (platform-specific Arduino API fallback)
#endif
