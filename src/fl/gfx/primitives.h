...
inline void addPixelToBuffer(CRGB* pixels, int width, int height,
                              int x, int y, const CRGB& color) {
...
/// Forward declarations for resolveCanvas implementation
inline DrawCanvas resolveCanvas(const RasterTarget& target) {
    if (target.template is<fl::Leds*>()) {
        fl::Leds* leds = target.template get<fl::Leds*>();
        return DrawCanvas{
            fl::span<CRGB>(leds->rgb(), leds->xymap().getTotal()),
            leds->xymap()
        };
    } else {
        return target.template get<DrawCanvas>();
    }
}
...
/// ============================================================================
/// Graphics operations work on rectangular Canvas objects with no XYMap indirection.
...
template<typename Coord>
void drawLine(gfx::Canvas<CRGB>& canvas,
              Coord x0, Coord y0, Coord x1, Coord y1, const CRGB& color);

/// @brief Antialiased filled circle
/// @param canvas Canvas with pixel buffer and dimensions
/// @param cx, cy Circle center coordinates (any arithmetic type)
/// @param r Circle radius (any arithmetic type)
/// @param color Pixel color
template<typename Coord>
void drawDisc(gfx::Canvas<CRGB>& canvas,
              Coord cx, Coord cy, Coord r, const CRGB& color);

/// @brief Antialiased circle outline
/// @param canvas Canvas with pixel buffer and dimensions
/// @param cx, cy Circle center coordinates (any arithmetic type)
/// @param r Inner radius of the ring (any arithmetic type)
/// @param thickness Ring thickness (any arithmetic type)
/// @param color Pixel color
template<typename Coord>
void drawRing(gfx::Canvas<CRGB>& canvas,
              Coord cx, Coord cy, Coord r, Coord thickness, const CRGB& color);

/// @brief Antialiased stroked line with configurable end caps
/// @param canvas Canvas with pixel buffer and dimensions
/// @param x0, y0, x1, y1 Line start and end coordinates (any arithmetic type: float, int, s16x16, etc)
/// @param thickness Line thickness (diameter)
/// @param color Pixel color
/// @param cap Line cap style (FLAT, ROUND, SQUARE)
template<typename Coord>
void drawStrokeLine(gfx::Canvas<CRGB>& canvas,
                    Coord x0, Coord y0, Coord x1, Coord y1, Coord thickness, const CRGB& color,
                    LineCap cap);
...
template<typename Coord>
inline void drawLine(gfx::Canvas<CRGB>& canvas,
                     Coord x0, Coord y0, Coord x1, Coord y1, const CRGB& color) {
...
template<typename Coord>
inline void drawDisc(gfx::Canvas<CRGB>& canvas,
                     Coord cx, Coord cy, Coord r, const CRGB& color) {
    CRGB* pixels = canvas.pixels;
    int width = canvas.width;
    int height = canvas.height;
...
template<typename Coord>
inline void drawRing(gfx::Canvas<CRGB>& canvas,
                     Coord cx, Coord cy, Coord r, Coord thickness, const CRGB& color) {
    CRGB* pixels = canvas.pixels;
    int width = canvas.width;
    int height = canvas.height;
...
template<typename Coord>
inline void drawStrokeLine(gfx::Canvas<CRGB>& canvas,
                           Coord x0, Coord y0, Coord x1, Coord y1, Coord thickness, const CRGB& color,
                           LineCap cap) {
...
/// ============================================================================
/// LEGACY RASTERTARGET IMPLEMENTATIONS
/// ============================================================================

/// @brief Wu antialiased line drawing
/// @param t RasterTarget (Leds* or DrawCanvas)
/// @param x0, y0, x1, y1 Line endpoints (template supports any arithmetic type)
/// @param color Line color
///
/// Algorithm: Wu's antialiasing with specialized code paths for steep vs. flat lines.
/// Two separate paths eliminate per-pixel branch misprediction on coordinate swaps.
///
/// Partially off-screen lines render correctly via bounds checking in DrawCanvas.
/// Off-screen lines (fully outside display) are silently clipped and render nothing.
template<typename Coord>
void drawLine(RasterTarget& target, Coord x0, Coord y0, Coord x1, Coord y1, const CRGB& color) {
    DrawCanvas canvas = resolveCanvas(target);
    ...
    if (steep) {
        // STEEP path: x and y coordinates swapped in pixel writes
        {
            ...
            CRGB c = color;
            c.nscale8(coordToU8(b1));
            canvas.addPixel(toInt(ypxl1),     toInt(xpxl1), c);
            c = color;
            c.nscale8(coordToU8(b2));
            canvas.addPixel(toInt(ypxl1+one), toInt(xpxl1), c);
        }
        Coord intery = yend + gradient;
        for (Coord x = loopStart; x < loopEnd; x = x + one) {
            Coord intery_floor = fl::floor(intery);
            ...
            CRGB c = color;
            c.nscale8(coordToU8(one - yf));
            canvas.addPixel(toInt(yint),     toInt(x), c);
            c = color;
            c.nscale8(coordToU8(yf));
            canvas.addPixel(toInt(yint+one), toInt(x), c);
            intery = intery + gradient;
        }
        {
            Coord yend2_floor = fl::floor(yend2);
            ...
            CRGB c = color;
            c.nscale8(coordToU8(b1));
            canvas.addPixel(toInt(ypxl2),     toInt(xpxl2), c);
            c = color;
            c.nscale8(coordToU8(b2));
            canvas.addPixel(toInt(ypxl2+one), toInt(xpxl2), c);
        }
    } else {
        // FLAT path: normal coordinate order
        {
            ...
            CRGB c = color;
            c.nscale8(coordToU8(b1));
            canvas.addPixel(toInt(xpxl1), toInt(ypxl1),     c);
            c = color;
            c.nscale8(coordToU8(b2));
            canvas.addPixel(toInt(xpxl1), toInt(ypxl1+one), c);
        }
        Coord intery = yend + gradient;
        for (Coord x = loopStart; x < loopEnd; x = x + one) {
            Coord intery_floor = fl::floor(intery);
            ...
            CRGB c = color;
            c.nscale8(coordToU8(one - yf));
            canvas.addPixel(toInt(x), toInt(yint),     c);
            c = color;
            c.nscale8(coordToU8(yf));
            canvas.addPixel(toInt(x), toInt(yint+one), c);
            intery = intery + gradient;
        }
        {
            Coord yend2_floor = fl::floor(yend2);
            ...
            CRGB c = color;
            c.nscale8(coordToU8(b1));
            canvas.addPixel(toInt(xpxl2), toInt(ypxl2),     c);
            c = color;
            c.nscale8(coordToU8(b2));
            canvas.addPixel(toInt(xpxl2), toInt(ypxl2+one), c);
        }
    }
}

/// @brief Antialiased filled circle using direct distance calculation
/// @param t RasterTarget
/// @param cx, cy Circle center coordinates
/// @param r Circle radius
/// @param color Circle color
///
/// Uses direct distance calculations with 0.5px soft AA fringe.
/// Partially off-screen circles render correctly via bounds checking.
template<typename Coord>
void drawDisc(RasterTarget& target, Coord cx, Coord cy, Coord r, const CRGB& color) {
    DrawCanvas canvas = resolveCanvas(target);
    ...
    Coord w = fromInt<Coord>(cw);
    Coord h = fromInt<Coord>(ch);
    ...
    for (Coord py = ymin; py <= ymax; py = py + one) {
        for (Coord px = xmin; px <= xmax; px = px + one) {
            ...
            if (d2 < r_outer2) {
                if (d2 < r_inner2) {
                    canvas.addPixel(toInt(px), toInt(py), color);
                } else {
                    Coord alpha = aaRatio(r_outer2 - d2, band, inv_band);
                    CRGB c = color;
                    c.nscale8(coordToU8(alpha));
                    canvas.addPixel(toInt(px), toInt(py), c);
                }
            }
        }
    }
...
/// @brief Antialiased circle outline with variable thickness
/// @param t RasterTarget
...
///
/// The ring spans [r, r+thickness] with 0.5px soft AA fringe on both edges.
/// When thickness >= 2*r the hole disappears and a filled circle is rendered.
template<typename Coord>
void drawRing(RasterTarget& target, Coord cx, Coord cy, Coord r, Coord thickness, const CRGB& color) {
    DrawCanvas canvas = resolveCanvas(target);
...
/// @param cap Line cap style (FLAT, ROUND, SQUARE)
///
/// Uses perpendicular-distance rasterization with per-row x-strip optimization.
/// Only visits pixels within the line's thickness, independent of line length or angle.
template<typename Coord>
void drawStrokeLine(RasterTarget& target, Coord x0, Coord y0, Coord x1, Coord y1, Coord thickness, const CRGB& color,
    ...
    int cw = canvas.width();
    int ch = canvas.height();
    Coord width_coord = fromInt<Coord>(cw);
    Coord height_coord = fromInt<Coord>(ch);
    Coord one = fromInt<Coord>(1);
    if (xmin_coord < zero) xmin_coord = zero;
    if (xmax_coord >= width_coord) xmax_coord = width_coord - one;
    if (ymin_coord < zero) ymin_coord = zero;
    if (ymax_coord >= height_coord) ymax_coord = height_coord - one;
        ...
        for (Coord px = xmin_coord; px <= xmax_coord; px = px + one) {
            ...
            if (dist2 < r_max2) {
                ...
                if (t >= zero && t <= one) {
                    ...
                    CRGB c = color;
                    c.nscale8(FL_PGM_READ_BYTE_NEAR(&detail::distanceAA_LUT[idx]));
                    canvas.addPixel(px_int, py_int, c);
                } else if (cap == LineCap::ROUND) {
                    // Hemispherical cap: distance from nearest endpoint
                    Coord ex = (t < zero) ? (px - x0) : (px - x1);
                    Coord ey = (t < zero) ? (py - y0) : (py - y1);
