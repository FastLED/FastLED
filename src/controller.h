...
protected:
    ...
    virtual void show(const PixelControllerArgs& args) = 0;
...
public:
    ...
    void show(const CRGB *data, int nLeds, uint8_t brightness) {
        // show(data, nLeds, getAdjustment(brightness));
        PixelControllerArgs args(
            data, nLeds, getAdjustment(brightness), BINARY_DITHER
        );
        show(args);
    }
...
protected:
    ...
    virtual void show(const struct CRGB *data, int nLeds, CRGB scale_pre_mixed) {
        PixelController<RGB_ORDER, LANES, MASK> pixels(PixelControllerArgs(data, nLeds < 0 ? -nLeds : nLeds, scale_pre_mixed, getDither()));
        if(nLeds < 0) {
