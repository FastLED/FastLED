...
VideoImpl::VideoImpl(size_t pixelsPerFrame, float fpsVideo,
    ...
    : mPixelsPerFrame(pixelsPerFrame),
      mFrameInterpolator(
          fl::make_intrusive<FrameInterpolator>(MAX(1, nFramesInBuffer), fpsVideo)) {}

void VideoImpl::pause(fl::u32 now) {
    if (!mTime) {
        mTime = fl::make_intrusive<TimeWarp>(now);
    }
    mTime->pause(now);
}
...
void VideoImpl::begin(FileHandlePtr h) {
    ...
    mStream = fl::make_intrusive<PixelStream>(mPixelsPerFrame * kSizeRGB8);
    mStream->begin(h);
    mPrevNow = 0;
}
...
bool VideoImpl::updateBufferFromStream(fl::u32 now) {
    ...
    for (size_t i = 0; i < frame_numbers.size(); ++i) {
        ...
        if (!recycled_frame) {
            ...
            recycled_frame = fl::make_intrusive<Frame>(mPixelsPerFrame);
        }
