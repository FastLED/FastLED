#pragma once

// IWYU pragma: private

// ok no namespace fl

/// @file platforms/avr/ldf_headers.h
/// AVR PlatformIO Library Dependency Finder (LDF) hints
///
/// This file contains #if 0 blocks with library includes to hint dependencies
/// to PlatformIO's LDF scanner without actually compiling the code.

// AVR platform uses Arduino framework - no additional LDF hints needed currently
// Add library includes here if LDF issues are discovered
