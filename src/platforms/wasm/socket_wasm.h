...
ssize_t sendmsg(int sockfd, const struct msghdr *msg, int flags) FL_NO_EXCEPT;
ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags) FL_NO_EXCEPT;
...
int close(int fd) FL_NO_EXCEPT;  // Uses shutdown() for sockets, real close() for other FDs
...
int getaddrinfo(const char *node, const char *service, 
                const struct addrinfo *hints, struct addrinfo **res) FL_NO_EXCEPT;
void freeaddrinfo(struct addrinfo *res) FL_NO_EXCEPT;
int getnameinfo(const struct sockaddr *sa, socklen_t salen,
                char *host, socklen_t hostlen, char *serv, socklen_t servlen, int flags) FL_NO_EXCEPT;
...
int ioctl(int fd, unsigned long request, ...) FL_NO_EXCEPT;
...
bool initialize_wasm_sockets() FL_NO_EXCEPT;

// Cleanup WASM socket subsystem (if needed) 
void cleanup_wasm_sockets() FL_NO_EXCEPT;

// Set mock behavior for testing (no-op in real implementation)
void set_wasm_socket_mock_behavior(bool should_fail, int error_code = ECONNREFUSED) FL_NO_EXCEPT;
...
WasmSocketStats get_wasm_socket_stats() FL_NO_EXCEPT;
void reset_wasm_socket_stats() FL_NO_EXCEPT;
...
public:
    explicit WasmSocket(const SocketOptions& options = {}) FL_NO_EXCEPT;
    ~WasmSocket() override = default;
    
    // Socket interface implementation
    fl::future<SocketError> connect(const fl::string& host, int port) FL_NO_EXCEPT override;
    fl::future<SocketError> connect_async(const fl::string& host, int port) FL_NO_EXCEPT override;
    void disconnect() FL_NO_EXCEPT override;
    bool is_connected() const FL_NO_EXCEPT override;
    SocketState get_state() const FL_NO_EXCEPT override;
    
    fl::size read(fl::span<u8> buffer) FL_NO_EXCEPT override;
    fl::size write(fl::span<const u8> data) FL_NO_EXCEPT override;
    fl::size available() const FL_NO_EXCEPT override;
    void flush() FL_NO_EXCEPT override;
    
    bool has_data_available() const FL_NO_EXCEPT override;
    bool can_write() const FL_NO_EXCEPT override;
    void set_non_blocking(bool non_blocking) FL_NO_EXCEPT override;
    bool is_non_blocking() const FL_NO_EXCEPT override;
    
    void set_timeout(u32 timeout_ms) FL_NO_EXCEPT override;
    u32 get_timeout() const FL_NO_EXCEPT override;
    void set_keep_alive(bool enable) FL_NO_EXCEPT override;
    void set_nodelay(bool enable) FL_NO_EXCEPT override;
    
    fl::string remote_address() const FL_NO_EXCEPT override;
    int remote_port() const FL_NO_EXCEPT override;
    fl::string local_address() const FL_NO_EXCEPT override;
    int local_port() const FL_NO_EXCEPT override;
    
    SocketError get_last_error() const FL_NO_EXCEPT override;
    fl::string get_error_message() const FL_NO_EXCEPT override;
    
    bool set_socket_option(int level, int option, const void* value, fl::size value_size) FL_NO_EXCEPT override;
    bool get_socket_option(int level, int option, void* value, fl::size* value_size) FL_NO_EXCEPT override;
    
    int get_socket_handle() const FL_NO_EXCEPT override;
    
protected:
    void set_state(SocketState state) FL_NO_EXCEPT override;
    void set_error(SocketError error, const fl::string& message = "") FL_NO_EXCEPT override;
...
private:
    ...
    SocketError translate_errno_to_socket_error(int error_code) FL_NO_EXCEPT;
    bool setup_socket_options() FL_NO_EXCEPT;
};

// Platform-specific socket creation functions (required by socket_factory.cpp)
fl::shared_ptr<Socket> create_platform_socket(const SocketOptions& options) FL_NO_EXCEPT;

// Platform capability queries  
bool platform_supports_ipv6() FL_NO_EXCEPT;
bool platform_supports_tls() FL_NO_EXCEPT;
bool platform_supports_non_blocking_connect() FL_NO_EXCEPT;
bool platform_supports_socket_reuse() FL_NO_EXCEPT;
