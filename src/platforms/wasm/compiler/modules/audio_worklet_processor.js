...
class FastLEDAudioProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    if (this.debugCounter % 500 === 0) {
      console.log(`🎵 FastLEDAudioProcessor: 🔄 process() called ${this.debugCounter} times, isProcessing: ${this.isProcessing}, inputs.length: ${inputs.length}`);
    }
    ...
    if (!this.isProcessing || inputs.length === 0 || inputs[0].length === 0) {
      if (this.debugCounter % 1000 === 0 && this.debugCounter > 0) {
        console.log(`🎵 FastLEDAudioProcessor: ⏭️ Skipping processing - isProcessing: ${this.isProcessing}, inputs: ${inputs.length}`);
      }
      return true;
    }
    ...
    if (inputChannel && inputChannel.length === 512) {
      ...
      if (this.debugCounter % 500 === 0) {
        ...
        console.log(`🎵 FastLEDAudioProcessor: 📤 Sending audio data, timestamp: ${timestamp}ms, avg sample magnitude: ${avgSample.toFixed(1)}`);
      }
      ...
      console.log(`🎵 FastLEDAudioProcessor: ❌ Invalid input channel - length: ${inputChannel ? inputChannel.length : 'null'}`);
    }
