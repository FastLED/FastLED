...
static void initWasmAudio(const char* name, WasmAudioInput*& wasmInput,
    ...
    if (!wasmInput) {
        ...
        if (wasmInputOwner) {
            ...
            ownsInput = true;
            FL_WARN("WasmAudioImpl: Created and started WasmAudioInput for '%s'", name);
        } else {
            FL_WARN("WasmAudioImpl: Failed to create WasmAudioInput: %s", error.c_str());
        }
    } else {
        // Ensure the existing input is started
        wasmInput->start();
        FL_WARN("WasmAudioImpl: Using existing WasmAudioInput for '%s'", name);
    }
}
