#pragma once

/// @file init_spi_hw_2.h
/// @brief Stub platform SpiHw2 initialization for testing
///
/// This header provides lazy initialization for stub/mock SpiHw2 instances
/// used in testing environments.

// allow-include-after-namespace

#if defined(FASTLED_TESTING) || defined(FASTLED_STUB_IMPL)

namespace fl {
namespace platform {

/// @brief Initialize stub SpiHw2 instances for testing
///
/// Registers mock SpiHw2 controller instances for testing.
/// Implementation is in src/platforms/stub/spi_2_stub.cpp
void initSpiHw2Instances();

}  // namespace platform
}  // namespace fl

#elif !defined(FASTLED_TESTING) && !defined(FASTLED_STUB_IMPL)

// When not in testing mode, use the default no-op implementation
#include "platforms/shared/init_spi_hw_2.h"

#endif  // defined(FASTLED_TESTING) || defined(FASTLED_STUB_IMPL)
