...
template <uint8_t PIN, uint8_t PAD> class _APOLLO3PIN {
  public:
    ...
    inline static void setOutput() {
        pinMode(PIN, OUTPUT);
        am_hal_gpio_fastgpio_enable(PAD);
    }
    ...
    inline static void toggle() __attribute__((always_inline)) {
        if (am_hal_gpio_fastgpio_read(PAD)) {
            lo();
        } else {
            hi();
        }
...
// to avoid having to use ap3_gpio_pin2pad for fastgpio (which would slow things
// down)
#define _FL_DEFPIN(PIN, PAD)                                                   \
    template <> class FastPin<PIN> : public _APOLLO3PIN<PIN, PAD> {};
