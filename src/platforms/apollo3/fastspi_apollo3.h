...
// It uses fastgpio instead of actual SPI, which means you can use it on all
// pins. It can run slightly faster than the default fastpin (bit banging).
...
class APOLLO3HardwareSPIOutput {
  ...
  public:
    ...
    template <uint8_t BIT> inline static void writeBit(uint8_t b) {
        ...
        for (uint32_t d = (_SPI_CLOCK_DIVIDER >> 1); d > 0; d--) {
            __NOP();
        }
        FastPin<_CLOCK_PIN>::lo();
        for (uint32_t d = (_SPI_CLOCK_DIVIDER >> 1); d > 0; d--) {
