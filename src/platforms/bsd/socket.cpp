...
bool BsdSocket::s_networking_initialized = false;
int BsdSocket::s_instance_count = 0;
fl::mutex BsdSocket::s_init_mutex;

//=============================================================================
// Static Platform Helpers
//=============================================================================

bool BsdSocket::initialize_networking() {
...
void BsdSocket::cleanup_networking() {
    fl::lock_guard<fl::mutex> lock(s_init_mutex);
...
fl::string BsdSocket::get_socket_error_string(int error_code) {
    return platform_get_socket_error_string(error_code);
}

SocketError BsdSocket::translate_socket_error(int error_code) {
...
BsdSocket::BsdSocket(const SocketOptions& options) 
...
fl::future<SocketError> BsdSocket::connect(const fl::string& host, int port) {
    auto error = connect_internal(host, port);
    return fl::make_ready_future(error);
}

fl::future<SocketError> BsdSocket::connect_async(const fl::string& host, int port) {
    // For now, same as synchronous connect
    return connect(host, port);
}

SocketError BsdSocket::connect_internal(const fl::string& host, int port) {
...
bool BsdSocket::is_connected() const {
    return mState == SocketState::CONNECTED && mSocket != INVALID_SOCKET_HANDLE;
}

SocketState BsdSocket::get_state() const {
    return mState;
}

fl::size BsdSocket::read(fl::span<fl::u8> buffer) {
...
fl::size BsdSocket::write(fl::span<const fl::u8> data) {
...
fl::size BsdSocket::available() const {
    if (!is_connected()) {
        return 0;
    }
...
void BsdSocket::flush() {
    // TCP sockets don't require explicit flushing
}

bool BsdSocket::has_data_available() const {
    return available() > 0;
}

bool BsdSocket::can_write() const {
    return is_connected();
}

void BsdSocket::set_non_blocking(bool non_blocking) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return;
    }
...
bool BsdSocket::is_non_blocking() const {
    return mIsNonBlocking;
}

void BsdSocket::set_timeout(fl::u32 timeout_ms) {
    mTimeout = timeout_ms;
...
fl::u32 BsdSocket::get_timeout() const {
    return mTimeout;
}

void BsdSocket::set_keep_alive(bool enable) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return;
    }
...
void BsdSocket::set_nodelay(bool enable) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return;
    }
...
fl::string BsdSocket::remote_address() const {
    return mRemoteHost;
}

int BsdSocket::remote_port() const {
    return mRemotePort;
}

fl::string BsdSocket::local_address() const {
    return mLocalAddress;
}

int BsdSocket::local_port() const {
    return mLocalPort;
}

SocketError BsdSocket::get_last_error() const {
    return mLastError;
}

fl::string BsdSocket::get_error_message() const {
    return mErrorMessage;
}

bool BsdSocket::set_socket_option(int level, int option, const void* value, fl::size value_size) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return false;
    }
...
bool BsdSocket::get_socket_option(int level, int option, void* value, fl::size* value_size) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return false;
    }
...
int BsdSocket::get_socket_handle() const {
    return static_cast<int>(mSocket);
}

void BsdSocket::set_state(SocketState state) {
    mState = state;
}

void BsdSocket::set_error(SocketError error, const fl::string& message) {
    mLastError = error;
...
bool BsdSocket::setup_socket_options() {
...
fl::string BsdSocket::resolve_hostname(const fl::string& hostname) {
...
//=============================================================================
// BsdServerSocket Implementation  
//=============================================================================

BsdServerSocket::BsdServerSocket(const SocketOptions& options) : mOptions(options) {
...
SocketError BsdServerSocket::bind(const fl::string& address, int port) {
...
SocketError BsdServerSocket::listen(int backlog) {
    mBacklog = backlog;
    mIsListening = true;
    return SocketError::SUCCESS;
}

void BsdServerSocket::close() {
    mIsListening = false;
}

bool BsdServerSocket::is_listening() const {
    return mIsListening;
}

fl::shared_ptr<Socket> BsdServerSocket::accept() {
    // Accept implementation would go here
    return nullptr;
}

fl::vector<fl::shared_ptr<Socket>> BsdServerSocket::accept_multiple(fl::size max_connections) {
    return fl::vector<fl::shared_ptr<Socket>>();
}

bool BsdServerSocket::has_pending_connections() const {
    return false;
}

void BsdServerSocket::set_reuse_address(bool enable) {
    (void)enable;
}

void BsdServerSocket::set_reuse_port(bool enable) {
    (void)enable;
}

void BsdServerSocket::set_non_blocking(bool non_blocking) {
    mIsNonBlocking = non_blocking;
}

fl::string BsdServerSocket::bound_address() const {
    return mBoundAddress;
}

int BsdServerSocket::bound_port() const {
    return mBoundPort;
}

fl::size BsdServerSocket::max_connections() const {
    return static_cast<fl::size>(mBacklog);
}

fl::size BsdServerSocket::current_connections() const {
    return mCurrentConnections;
}

SocketError BsdServerSocket::get_last_error() const {
    return mLastError;
}

fl::string BsdServerSocket::get_error_message() const {
    return mErrorMessage;
}

int BsdServerSocket::get_socket_handle() const {
    return static_cast<int>(mSocket);
}

void BsdServerSocket::set_error(SocketError error, const fl::string& message) {
    mLastError = error;
    mErrorMessage = message;
}
...
fl::shared_ptr<Socket> create_platform_socket(const SocketOptions& options) {
    return fl::make_shared<BsdSocket>(options);
}

fl::shared_ptr<ServerSocket> create_platform_server_socket(const SocketOptions& options) {
    return fl::make_shared<BsdServerSocket>(options);
}

bool platform_supports_ipv6() {
    return true;  // Most modern Unix systems support IPv6
}
