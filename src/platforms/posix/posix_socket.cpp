...
bool PosixSocket::s_networking_initialized = false;
int PosixSocket::s_instance_count = 0;
fl::mutex PosixSocket::s_init_mutex;

//=============================================================================
// Static Platform Helpers
//=============================================================================

bool PosixSocket::initialize_networking() {
...
void PosixSocket::cleanup_networking() {
    fl::lock_guard<fl::mutex> lock(s_init_mutex);
...
fl::string PosixSocket::get_socket_error_string(int error_code) {
    return platform_get_socket_error_string(error_code);
}

SocketError PosixSocket::translate_socket_error(int error_code) {
...
PosixSocket::PosixSocket(const SocketOptions& options) 
...
fl::future<SocketError> PosixSocket::connect(const fl::string& host, int port) {
    auto error = connect_internal(host, port);
    return fl::make_ready_future(error);
}

fl::future<SocketError> PosixSocket::connect_async(const fl::string& host, int port) {
    // For now, same as synchronous connect
    // In a full implementation, this would use non-blocking sockets and poll/select
    return connect(host, port);
}

SocketError PosixSocket::connect_internal(const fl::string& host, int port) {
...
bool PosixSocket::is_connected() const {
    return mState == SocketState::CONNECTED && mSocket != INVALID_SOCKET_HANDLE;
}

SocketState PosixSocket::get_state() const {
    return mState;
}

fl::size PosixSocket::read(fl::span<fl::u8> buffer) {
...
fl::size PosixSocket::write(fl::span<const fl::u8> data) {
...
fl::size PosixSocket::available() const {
    if (!is_connected()) {
        return 0;
    }
...
void PosixSocket::flush() {
    // TCP sockets don't require explicit flushing
}

bool PosixSocket::has_data_available() const {
    return available() > 0;
}

bool PosixSocket::can_write() const {
    return is_connected();
}
...
bool PosixSocket::is_non_blocking() const {
    return mIsNonBlocking;
}

void PosixSocket::set_timeout(fl::u32 timeout_ms) {
    mTimeout = timeout_ms;
...
fl::u32 PosixSocket::get_timeout() const {
    return mTimeout;
}

void PosixSocket::set_keep_alive(bool enable) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return;
    }
...
void PosixSocket::set_nodelay(bool enable) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return;
    }
...
fl::string PosixSocket::remote_address() const {
    return mRemoteHost;
}

int PosixSocket::remote_port() const {
    return mRemotePort;
}

fl::string PosixSocket::local_address() const {
    return mLocalAddress;
}

int PosixSocket::local_port() const {
    return mLocalPort;
}

SocketError PosixSocket::get_last_error() const {
    return mLastError;
}

fl::string PosixSocket::get_error_message() const {
    return mErrorMessage;
}

bool PosixSocket::set_socket_option(int level, int option, const void* value, fl::size value_size) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return false;
    }
...
bool PosixSocket::get_socket_option(int level, int option, void* value, fl::size* value_size) {
    socket_t platform_socket = to_platform_socket(mSocket);
    if (platform_socket == INVALID_SOCKET_VALUE) {
        return false;
    }
...
int PosixSocket::get_socket_handle() const {
    return static_cast<int>(mSocket);
}

void PosixSocket::set_state(SocketState state) {
    mState = state;
}

void PosixSocket::set_error(SocketError error, const fl::string& message) {
    mLastError = error;
...
bool PosixSocket::setup_socket_options() {
...
fl::string PosixSocket::resolve_hostname(const fl::string& hostname) {
...
bool platform_supports_ipv6() {
    return true;  // Most modern platforms support IPv6
}
