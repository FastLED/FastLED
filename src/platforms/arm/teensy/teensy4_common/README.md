...
- `clockless_arm_mxrt1062.h`: backward-compatible `ClocklessController_BitBang` alias onto the ObjectFLED slim bridge.
- `block_clockless_arm_mxrt1062.h`: multi-lane slim bridge over `ChannelEngineObjectFLED`; lane pin order lives in `block_lane_pins.h`.
- `block_lane_pins.h`: lane-to-pin order for the block clockless controller.
- `octows2811_controller.h`: OctoWS2811 integration (`USE_OCTOWS2811`), intentionally unbridged: the PJRC library owns its own DMA and framebuffers.
...
## Legacy `addLeds<>()` routing

All legacy Teensy 4 `addLeds<>()` clockless paths (single-lane, BitBang alias and
block/multi-lane) now go through `ChannelManager` via the ObjectFLED slim bridge
(#4588). The one exception is `COctoWS2811Controller`, which stays a direct wrapper
around the third-party OctoWS2811 library. Hardware validation is tracked in #4623.
...
## Notes
- Very high CPU frequency; clockless output is generated by the ObjectFLED DMA engine (FlexIO/DMA timing), so bit timing does not depend on CPU cycle counting or interrupt latency.
