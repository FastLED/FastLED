...
|----------|------|-----|---------------|-----------------|--------|
| **LPC845** | LPC845M301 | Cortex-M0+ @ 24 MHz | 24 MHz (FRO direct path) | Bit-bang (default) + optional SCT/DMA channels-API engine ([#3460](https://github.com/FastLED/FastLED/pull/3460), TX→RX byte-match [#3472](https://github.com/FastLED/FastLED/pull/3472), `addLeds<>` route [#3527](https://github.com/FastLED/FastLED/pull/3527)) + optional SPI DMA async ([#3454](https://github.com/FastLED/FastLED/pull/3454)) | ✅ Compiles + AutoResearch RPC bring-up sketch ([#3041](https://github.com/FastLED/FastLED/pull/3041)) + host-side TX→RX byte-match through `engine.show()` ([#3472](https://github.com/FastLED/FastLED/pull/3472)); on-silicon loopback pending [#2880](https://github.com/FastLED/FastLED/issues/2880) |
| **LPC804** | LPC804M101 | Cortex-M0+ @ 15 MHz | 15 MHz (FRO direct path) | Bit-bang (default) + optional PLU ([#2848](https://github.com/FastLED/FastLED/pull/2848)). **No DMA-async SPI** — LPC804 silicon has no DMA peripheral (NXP `mcux-sdk/devices/LPC804/LPC804.h` has zero `DMA_Type` / `DMA0_BASE`; UM11065 has no DMA chapter). Earlier gate widening in #3500 was reverted; see [`agents/docs/peripheral-existence.md`](../../../../agents/docs/peripheral-existence.md) "Historical anti-example". | ✅ Compiles + AutoResearch RPC bring-up sketch ([#3041](https://github.com/FastLED/FastLED/pull/3041)); AutoResearch loopback verification pending [#2880](https://github.com/FastLED/FastLED/issues/2880) |
...
- ~~`clockless_arm_lpc_pwm_dma.h`~~ — **retired** in #3517 Phase A.3. The classic `ClocklessController<>` template that supplied the SCT+DMA fast path is now the channels-API adapter `clockless_channel_lpc.h`, and `addLeds<WS2812>()` under `FASTLED_LPC_PWM_DMA=1` routes through `ChannelEngineLpcSctDma` via `BusTraits<Bus::BIT_BANG>`. Legacy header removed to avoid two-code-path divergence. Slimmed onto `fl::SlimBridgeController` under #4586: `ClocklessController<>` is now a thin subclass parameterized on `BusTraits<Bus::BIT_BANG>`, the constructor registers with `ChannelManager`, and per-frame `driver->show()` calls are gone — `ChannelManager`'s end-of-show flush drains the driver once per `FastLED.show()`.
...
- `spi_arm_lpc_dma.h` — **LPC845-only** hardware SPI + single-channel DMA async driver ([#3454](https://github.com/FastLED/FastLED/pull/3454)). Same MSB-first / mode-0 / 8-bit SPI framing as the polled `spi_arm_lpc.h`, but the TX byte stream is fed by one DMA0 channel (default: channel 4 for SPI0, override to 6 for SPI1). Opt-in via `FASTLED_LPC_SPI_DMA`; falls back to the polled driver otherwise. LPC804 is not supported — a compile-time `#error` fires if `FASTLED_LPC_SPI_DMA` is set on an LPC804 build (silicon has no DMA peripheral).
...
- `clockless_channel_lpc_uart_dma.h` — Legacy `addLeds<WS2812, PIN, GRB>()` adapter that selects the UART DMA channel engine by default on LPC845. Slimmed onto `fl::SlimBridgeController` under #4586, parameterized on `BusTraits<Bus::UART>`, alongside `clockless_channel_lpc.h`'s `BusTraits<Bus::BIT_BANG>` bridge — both are now thin `fl::SlimBridgeController` subclasses sharing one enqueue/wait/encode implementation instead of hand-rolled driver plumbing. Hardware validation on LPC845 silicon is still pending.
...
- ~~[#3500](https://github.com/FastLED/FastLED/pull/3500)~~ — **REVERTED.** Attempted to widen `spi_arm_lpc_dma.h` gate to LPC804. LPC804 silicon has no DMA peripheral; the "vendor DMA0 typedef" was a fabrication in `framework-arduino-lpc8xx#35` (also to be reverted). See #3506 for the guardrail that prevents recurrence and `agents/docs/peripheral-existence.md` for the empirical grep recipe.
