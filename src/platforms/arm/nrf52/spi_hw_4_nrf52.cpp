...
bool SPIQuadNRF52::transmitAsync(fl::span<const uint8_t> buffer) {
    if (!mInitialized) {
        return false;
    }
    ...
    fl::memcopy(mLane2Buffer, buffer.data() + (bytes_per_lane * 2), bytes_per_lane);
    fl::memcopy(mLane3Buffer, buffer.data() + (bytes_per_lane * 3), bytes_per_lane);
...
/// NRF52840/52833 factory override - returns available Quad-SPI instances
...
fl::vector<SpiHw4*> SpiHw4::createInstances() {
    ...
    static SPIQuadNRF52 controller0(0, "SPIM0+1+2+3");
