...
// NRF52840 adds SPIM3 (same frequencies), adds SPIM3 that can be @ up to 32Mbps
// frequency(!)
...
class NRF52SPIOutput {
  private:
    ...
    static bool s_InUse;
    static bool s_NeedToWait; // a data transfer was started, and completion
                              // event was not cleared.
    ...
    */
    ...
    static uint8_t
        s_Buffer[2][2]; // 2x two-byte buffers, allows one buffer currently
                        // being sent, and a second one being prepped to send.
  ...
  public:
    ...
    // Low frequency GPIO is for signals with a frequency up to 10 kHz.  Lowest
    // speed SPIM is 125kbps.
    static_assert(!FastPin<_DATA_PIN>::LowSpeedOnlyRecommended(),
                  "Invalid (low-speed only) pin specified");
    static_assert(!FastPin<_CLOCK_PIN>::LowSpeedOnlyRecommended(),
                  "Invalid (low-speed only) pin specified");

    /// initialize the SPI subssytem
    void init() {
        // 0. ASSERT() the SPIM instance is not enabled / in use
        // ASSERT(m_SPIM->ENABLE != (SPIM_ENABLE_ENABLE_Enabled <<
        ...
        // 2. Configure SPIMx
        nrf_spim_configure(FASTLED_NRF52_SPIM, NRF_SPIM_MODE_0,
                           NRF_SPIM_BIT_ORDER_MSB_FIRST);
        nrf_spim_frequency_set(
            FASTLED_NRF52_SPIM,
            NRF_SPIM_FREQ_4M // BUGBUG -- use _SPI_CLOCK_DIVIDER to determine
                             // frequency
        );
        nrf_spim_pins_set(FASTLED_NRF52_SPIM, FastPin<_CLOCK_PIN>::nrf_pin(),
    ...
    // (caller must still waitFully() before actually starting this next
    // transaction)
    static void wait() {
        if (!s_NeedToWait)
            return;
        ...
        // leave the event set here... caller must waitFully() and start next
        // transaction
        return;
    }
    ...
    static void writeByte(uint8_t b) {
        ...
        s_Buffer[i][0u] = b; // cannot use the stack location, so copy to a more
                             // permanent buffer...
        nrf_spim_tx_buffer_set(FASTLED_NRF52_SPIM, &(s_Buffer[i][0u]), 1);

        waitFully();
        nrf_spim_task_trigger(FASTLED_NRF52_SPIM, NRF_SPIM_TASK_START);
        return;
    }
    ...
    static void writeWord(uint16_t w) {
        ...
        s_Buffer[i][0u] = (w >> 8u);   // cannot use the stack location, so copy
                                       // to a more permanent buffer...
        s_Buffer[i][1u] = (w & 0xFFu); // cannot use the stack location, so copy
                                       // to a more permanent buffer...
        nrf_spim_tx_buffer_set(FASTLED_NRF52_SPIM, &(s_Buffer[i][0u]), 2);
    ...
    /// A raw set of writing byte values, assumes setup/init/waiting done
    /// elsewhere (static for use by adjustment classes)
    ...
    /// A full cycle of writing a value for len bytes, including select,
    /// release, and waiting
    ...
    /// A full cycle of writing a raw block of data out, including select,
    /// release, and waiting
    void writeBytes(uint8_t *data, int len) {
        // This is a special-case, with no adjustment of the bytes... write them
        // directly...
        select();
        wait();
        nrf_spim_tx_buffer_set(FASTLED_NRF52_SPIM, data, len);
        waitFully();
        nrf_spim_task_trigger(FASTLED_NRF52_SPIM, NRF_SPIM_TASK_START);
        waitFully();
        release();
    ...
    /// write a single bit out, which bit from the passed in byte is determined
    ...
    /// write out pixel data from the given PixelController object, including
    /// select, release, and waiting
...
// see
// https://stackoverflow.com/questions/3229883/static-member-initialization-in-a-class-template#answer-3229919
template <uint8_t _DATA_PIN, uint8_t _CLOCK_PIN, uint32_t _SPI_CLOCK_DIVIDER>
bool NRF52SPIOutput<_DATA_PIN, _CLOCK_PIN, _SPI_CLOCK_DIVIDER>::s_InUse = false;
template <uint8_t _DATA_PIN, uint8_t _CLOCK_PIN, uint32_t _SPI_CLOCK_DIVIDER>
bool NRF52SPIOutput<_DATA_PIN, _CLOCK_PIN, _SPI_CLOCK_DIVIDER>::s_NeedToWait =
    false;
template <uint8_t _DATA_PIN, uint8_t _CLOCK_PIN, uint32_t _SPI_CLOCK_DIVIDER>
uint8_t
    NRF52SPIOutput<_DATA_PIN, _CLOCK_PIN, _SPI_CLOCK_DIVIDER>::s_BufferIndex =
        0;
template <uint8_t _DATA_PIN, uint8_t _CLOCK_PIN, uint32_t _SPI_CLOCK_DIVIDER>
uint8_t NRF52SPIOutput<_DATA_PIN, _CLOCK_PIN,
                       _SPI_CLOCK_DIVIDER>::s_Buffer[2][2] = {{0, 0}, {0, 0}};
