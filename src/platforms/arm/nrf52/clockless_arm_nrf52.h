...
#define FASTLED_NRF52_MAXIMUM_PIXELS_PER_STRING                                \
    144 // TODO: Figure out how to safely let this be calller-defined....
...
template <uint8_t _DATA_PIN, int _T1, int _T2, int _T3, EOrder _RGB_ORDER = RGB,
          int _XTRA0 = 0, bool _FLIP = false, int _WAIT_TIME_MICROSECONDS = 10>
class ClocklessController : public CPixelLEDController<_RGB_ORDER> {
    static_assert(FASTLED_NRF52_MAXIMUM_PIXELS_PER_STRING > 0,
                  "Maximum string length must be positive value "
    ...
    static_assert(_T1 + _T2 + _T3 <= PWM_COUNTERTOP_COUNTERTOP_Msk,
                  "_TOP too large for peripheral");

  private:
    static const bool _INITIALIZE_PIN_HIGH = (_FLIP ? 1 : 0);
    static const uint16_t _POLARITY_BIT = (_FLIP ? 0 : 0x8000);

    static const uint8_t _BITS_PER_PIXEL =
        (8 + _XTRA0) * 3; // NOTE: 3 means RGB only...
    static const uint16_t _PWM_BUFFER_COUNT =
    ...
    // may as well be static, as can only attach one LED string per
    // _DATA_PIN....
    ...
    static CMinWait<_WAIT_TIME_MICROSECONDS>
        mWait; // ensure data has time to latch
    ...
    FASTLED_NRF52_INLINE_ATTRIBUTE static void
    startPwmPlayback_InitializePwmInstance(NRF_PWM_Type *pwm) {
    ...
    FASTLED_NRF52_INLINE_ATTRIBUTE static void
    startPwmPlayback_ConfigurePwmSequence(NRF_PWM_Type *pwm) {
        ...
        sequenceConfig.end_delay =
            0; // no extra delay at the end of SEQ[0] / SEQ[1]
        nrf_pwm_sequence_set(pwm, 0, &sequenceConfig);
        nrf_pwm_sequence_set(pwm, 1, &sequenceConfig);
        nrf_pwm_loop_set(pwm, 0);
    }
    FASTLED_NRF52_INLINE_ATTRIBUTE static void
    startPwmPlayback_EnableInterruptsAndShortcuts(NRF_PWM_Type *pwm) {
        IRQn_Type irqn = PWM_Arbiter<FASTLED_NRF52_PWM_ID>::getIRQn();
        // TODO: check API results...
        uint32_t result;

        result =
            sd_nvic_SetPriority(irqn, configMAX_SYSCALL_INTERRUPT_PRIORITY);
        (void)result;
        result = sd_nvic_EnableIRQ(irqn);
        (void)result;

        // shortcuts prevent (up to) 4-cycle delay from interrupt handler to
        // next action
        uint32_t shortsToEnable = 0;
        shortsToEnable |=
            NRF_PWM_SHORT_SEQEND0_STOP_MASK; ///< SEQEND[0] --> STOP task.
        shortsToEnable |=
            NRF_PWM_SHORT_SEQEND1_STOP_MASK; ///< SEQEND[1] --> STOP task.
        // shortsToEnable |= NRF_PWM_SHORT_LOOPSDONE_SEQSTART0_MASK; ///<
        // LOOPSDONE --> SEQSTART[0] task. shortsToEnable |=
        // NRF_PWM_SHORT_LOOPSDONE_SEQSTART1_MASK; ///< LOOPSDONE -->
        // SEQSTART[1] task.
        shortsToEnable |=
            NRF_PWM_SHORT_LOOPSDONE_STOP_MASK; ///< LOOPSDONE --> STOP task.
    ...
    FASTLED_NRF52_INLINE_ATTRIBUTE static void
    startPwmPlayback_StartTask(NRF_PWM_Type *pwm) {
    ...
    FASTLED_NRF52_INLINE_ATTRIBUTE static void releaseSequenceBuffer() {
        uint32_t tmp =
            __sync_val_compare_and_swap(&s_SequenceBufferInUse, 1, 0);
        if (tmp != 1) {
  ...
  public:
    static void isr_handler() {
        ...
        if (nrf_pwm_event_check(pwm, NRF_PWM_EVENT_STOPPED)) {
            ...
            mWait.mark();
            // mark the sequence as no longer in use -- pointer, comparator,
            // exchange value
    ...
    virtual void init() {
        ...
        // to avoid pin initialization from causing first LED to have invalid
        // color, call mWait.mark() to ensure data latches before color data
        // gets sent.
        startPwmPlayback_InitializePinState();
        mWait.mark();
    }
    ...
    FASTLED_NRF52_INLINE_ATTRIBUTE static void WriteBitToSequence(uint8_t byte,
                                                                  uint16_t *e) {
        *e = _POLARITY_BIT | (((byte & (1u << _BIT)) == 0) ? _T0H : _T1H);
    }
    FASTLED_NRF52_INLINE_ATTRIBUTE static void
    prepareSequenceBuffers(PixelController<_RGB_ORDER> &pixels) {
        ...
        while (pixels.has(1) &&
            ...
            WriteBitToSequence<7>(b0, e);
            ++e;
            WriteBitToSequence<6>(b0, e);
            ++e;
            WriteBitToSequence<5>(b0, e);
            ++e;
            WriteBitToSequence<4>(b0, e);
            ++e;
            WriteBitToSequence<3>(b0, e);
            ++e;
            WriteBitToSequence<2>(b0, e);
            ++e;
            WriteBitToSequence<1>(b0, e);
            ++e;
            WriteBitToSequence<0>(b0, e);
            ++e;
            if (_XTRA0 > 0) {
                for (int i = 0; i < _XTRA0; ++i) {
                    WriteBitToSequence<0>(0, e);
                    ++e;
                }
            }
            uint8_t b1 = pixels.loadAndScale1();
            WriteBitToSequence<7>(b1, e);
            ++e;
            WriteBitToSequence<6>(b1, e);
            ++e;
            WriteBitToSequence<5>(b1, e);
            ++e;
            WriteBitToSequence<4>(b1, e);
            ++e;
            WriteBitToSequence<3>(b1, e);
            ++e;
            WriteBitToSequence<2>(b1, e);
            ++e;
            WriteBitToSequence<1>(b1, e);
            ++e;
            WriteBitToSequence<0>(b1, e);
            ++e;
            if (_XTRA0 > 0) {
                for (int i = 0; i < _XTRA0; ++i) {
                    WriteBitToSequence<0>(0, e);
            ...
            WriteBitToSequence<7>(b2, e);
            ++e;
            WriteBitToSequence<6>(b2, e);
            ++e;
            WriteBitToSequence<5>(b2, e);
            ++e;
            WriteBitToSequence<4>(b2, e);
            ++e;
            WriteBitToSequence<3>(b2, e);
            ++e;
            WriteBitToSequence<2>(b2, e);
            ++e;
            WriteBitToSequence<1>(b2, e);
            ++e;
            WriteBitToSequence<0>(b2, e);
            ++e;
            if (_XTRA0 > 0) {
                for (int i = 0; i < _XTRA0; ++i) {
                    WriteBitToSequence<0>(0, e);
    ...
    FASTLED_NRF52_INLINE_ATTRIBUTE static void
    startPwmPlayback(uint16_t bytesToSend) {
...
template <uint8_t _DATA_PIN, int _T1, int _T2, int _T3, EOrder _RGB_ORDER,
          int _XTRA0, bool _FLIP, int _WAIT_TIME_MICROSECONDS>
uint16_t ClocklessController<
    _DATA_PIN, _T1, _T2, _T3, _RGB_ORDER, _XTRA0, _FLIP,
    _WAIT_TIME_MICROSECONDS>::s_SequenceBufferValidElements = 0;
template <uint8_t _DATA_PIN, int _T1, int _T2, int _T3, EOrder _RGB_ORDER,
          int _XTRA0, bool _FLIP, int _WAIT_TIME_MICROSECONDS>
uint32_t volatile ClocklessController<
    _DATA_PIN, _T1, _T2, _T3, _RGB_ORDER, _XTRA0, _FLIP,
    _WAIT_TIME_MICROSECONDS>::s_SequenceBufferInUse = 0;
template <uint8_t _DATA_PIN, int _T1, int _T2, int _T3, EOrder _RGB_ORDER,
          int _XTRA0, bool _FLIP, int _WAIT_TIME_MICROSECONDS>
uint16_t ClocklessController<
    _DATA_PIN, _T1, _T2, _T3, _RGB_ORDER, _XTRA0, _FLIP,
    _WAIT_TIME_MICROSECONDS>::s_SequenceBuffer[_PWM_BUFFER_COUNT];
template <uint8_t _DATA_PIN, int _T1, int _T2, int _T3, EOrder _RGB_ORDER,
          int _XTRA0, bool _FLIP, int _WAIT_TIME_MICROSECONDS>
CMinWait<_WAIT_TIME_MICROSECONDS>
    ClocklessController<_DATA_PIN, _T1, _T2, _T3, _RGB_ORDER, _XTRA0, _FLIP,
                        _WAIT_TIME_MICROSECONDS>::mWait;
...
// (per softdevice 1.40, see
http://infocenter.nordicsemi.com/pdf/S140_SDS_v1.3.pdf)
//
// The nRF52 chips have a decent Easy-DMA-enabled PWM peripheral.
//
// The major downside:
// [] The PWM peripheral has a fixed input buffer size at 16 bits per clock
cycle.
...
// apis to send the pre-generated buffer.  This would be essentially
asynchronous,
