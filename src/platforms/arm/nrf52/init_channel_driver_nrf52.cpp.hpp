...
static void addSpiHardwareIfPossible(ChannelManager& manager) {
    ...
    FL_DBG("NRF52: Found %s SpiHw4 controllers", hw4Controllers.size());
    ...
    FL_DBG("NRF52: Found %s SpiHw2 controllers", hw2Controllers.size());
    ...
    if (!controllers.empty()) {
        ...
        if (adapter) {
            ...
            FL_DBG("NRF52: Registered unified SPI driver with %s controllers (priority %s)", controllers.size(), maxPriority);
        } else {
