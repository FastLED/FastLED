...
// Definition for a single channel clockless controller for the k66 family of
// chips, like that used in the teensy 3.6 See clockless.h for detailed info on
// how the template parameters are used.
...
template <uint8_t LANES, int FIRST_PIN, int T1, int T2, int T3,
          EOrder RGB_ORDER = GRB, int XTRA0 = 0, bool FLIP = false,
          int WAIT_TIME = 40>
class InlineBlockClocklessController
    : public CPixelLEDController<RGB_ORDER, LANES, LANE_MASK> {
  ...
  public:
    ...
    virtual void
    showPixels(PixelController<RGB_ORDER, LANES, LANE_MASK> &pixels) {
        mWait.wait();
    ...
    virtual void init() {
        if (FIRST_PIN == PORTC_FIRST_PIN) { // PORTC
            ...
            case 4:
                FastPin<9>::setOutput();
            case 3:
                FastPin<23>::setOutput();
            case 2:
                FastPin<22>::setOutput();
            case 1:
                FastPin<15>::setOutput();
            }
            ...
            switch (USED_LANES) {
            case 8:
                FastPin<45>::setOutput();
            case 7:
                FastPin<44>::setOutput();
            case 6:
                FastPin<46>::setOutput();
            case 5:
                FastPin<43>::setOutput();
            case 4:
                FastPin<30>::setOutput();
            case 3:
                FastPin<29>::setOutput();
    ...
    writeBits(FASTLED_REGISTER uint32_t &next_mark, FASTLED_REGISTER Lines &b,
              PixelController<RGB_ORDER, LANES, LANE_MASK>
                  &pixels) { // , FASTLED_REGISTER uint32_t & b2)  {
        ...
        for (FASTLED_REGISTER uint32_t i = 0; i < (USED_LANES / 2); ++i) {
            ...
            *FastPin<FIRST_PIN>::sport() = PORT_MASK;
            ...
            while ((next_mark - ARM_DWT_CYCCNT) >
                   (T2 + T3 + (2 * (F_CPU / 24000000))))
                ;
            if (USED_LANES > 8) {
                *FastPin<FIRST_PIN>::cport() = ((~b2.shorts[i]) & PORT_MASK);
            } else {
                *FastPin<FIRST_PIN>::cport() =
                    (PORT_SHIFT(~b2.bytes[7 - i]) & PORT_MASK);
            }
            ...
            *FastPin<FIRST_PIN>::cport() = PORT_MASK;
            ...
            b.bytes[i + (USED_LANES / 2)] = pixels.template loadAndScale<PX>(
                pixels, i + (USED_LANES / 2), d, scale);
        }

        // if folks use an odd numnber of lanes, get the last byte's value here
        if (USED_LANES & 0x01) {
            b.bytes[USED_LANES - 1] = pixels.template loadAndScale<PX>(
                pixels, USED_LANES - 1, d, scale);
        }
    ...
    static uint32_t
    showRGBInternal(PixelController<RGB_ORDER, LANES, LANE_MASK> &allpixels) {
...
template <uint8_t LANES, int T1, int T2, int T3, EOrder RGB_ORDER = GRB,
          int XTRA0 = 0, bool FLIP = false, int WAIT_TIME = 50>
class SixteenWayInlineBlockClocklessController
    : public CPixelLEDController<RGB_ORDER, LANES, PMASK> {
  ...
  public:
    virtual void init() {
        static_assert(LANES <= 16,
                      "Maximum of 16 lanes for Teensy parallel controllers!");
        // FastPin<30>::setOutput();
        // FastPin<29>::setOutput();
        // FastPin<27>::setOutput();
        // FastPin<28>::setOutput();
    ...
    __attribute__((always_inline)) inline static void
    writeBits(FASTLED_REGISTER uint32_t &next_mark, FASTLED_REGISTER Lines &b,
              PixelController<RGB_ORDER, LANES, PMASK>
                  &pixels) { // , FASTLED_REGISTER uint32_t & b2)  {
        FASTLED_REGISTER Lines b2;
        transpose8x1(b.bytes, b2.bytes);
        ...
        for (FASTLED_REGISTER uint32_t i = 0; (i < LANES) && (i < 8); ++i) {
            ...
            *FastPin<PORTC_FIRST_PIN>::sport() = PMASK_HI;
            ...
            while ((next_mark - ARM_DWT_CYCCNT) > (T2 + T3 + 6))
                ;
            *FastPin<PORTD_FIRST_PIN>::cport() =
                ((~b2.bytes[7 - i]) & PMASK_LO);
            *FastPin<PORTC_FIRST_PIN>::cport() =
                ((~b2.bytes[15 - i]) & PMASK_HI);
            ...
            *FastPin<PORTC_FIRST_PIN>::cport() = PMASK_HI;
            ...
            if (LANES == 16 || (LANES > 8 && ((i + 8) < LANES))) {
                b.bytes[i + 8] =
                    pixels.template loadAndScale<PX>(pixels, i + 8, d, scale);
            }
        }
    }
    ...
    // Y for the this pointer.
    static uint32_t
    showRGBInternal(PixelController<RGB_ORDER, LANES, PMASK> &allpixels) {
        // Get access to the clock
        ARM_DEMCR |= ARM_DEMCR_TRCENA;
        ARM_DWT_CTRL |= ARM_DWT_CTRL_CYCCNTENA;
        ARM_DWT_CYCCNT = 0;
