...
// Definition for a single channel clockless controller for the k20 family of
// chips, like that used in the teensy 3.0/3.1 See clockless.h for detailed info
// on how the template parameters are used.
...
class InlineBlockClocklessController
    : public CPixelLEDController<RGB_ORDER, LANES, PORT_MASK> {
  ...
  public:
    ...
    virtual void
    showPixels(PixelController<RGB_ORDER, LANES, PORT_MASK> &pixels) {
        mWait.wait();
    ...
    writeBits(FASTLED_REGISTER uint32_t &next_mark, FASTLED_REGISTER Lines &b,
              PixelController<RGB_ORDER, LANES, PORT_MASK>
    ...
    static uint32_t
    showRGBInternal(PixelController<RGB_ORDER, LANES, PORT_MASK> &allpixels) {
