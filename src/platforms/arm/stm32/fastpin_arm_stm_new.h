...
// Register mapping for STM32F4 family (including F401)
#define _RD32_F4(T) struct __gen_struct_ ## T { \
    static __attribute__((always_inline)) FASTLED_FORCE_INLINE volatile GPIO_TypeDef* r() { return T; } \
};
#define _IO32(L) _RD32_F4(GPIO ## L)

// Most STM32F401 packages use up to GPIOA-E
_IO32(A); _IO32(B); _IO32(C); _IO32(D); _IO32(E);

// Example pin mapping for STM32F401, adjust as required for your board (Nucleo, BlackPill, etc)
#define MAX_PIN PA15

// Example pin-to-port and bit mapping for STM32F401 (Nucleo, BlackPill, BluePill F401)
_DEFPIN_ARM(PA0, 0, A);
_DEFPIN_ARM(PA1, 1, A);
_DEFPIN_ARM(PA2, 2, A);
_DEFPIN_ARM(PA3, 3, A);
...
_DEFPIN_ARM(PC13, 13, C); // Usually connected to user button or RTC LSE
_DEFPIN_ARM(PC14, 14, C); // RTC LSE
_DEFPIN_ARM(PC15, 15, C); // RTC LSE

_DEFPIN_ARM(PD2, 2, D); // Only some packages use D pins

// SPI handy macros for F401 (Common for Nucleo, BlackPill)
// SPI1: PA7 = MOSI, PA5 = SCK
#define SPI_DATA PA7
