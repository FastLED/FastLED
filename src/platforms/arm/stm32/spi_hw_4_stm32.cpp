...
bool SPIQuadSTM32::transmitAsync(fl::span<const uint8_t> buffer) {
    ...
    // Clear buffers
    memset(mDMABuffer0, 0, buffer_size_per_lane);
    memset(mDMABuffer1, 0, buffer_size_per_lane);
    memset(mDMABuffer2, 0, buffer_size_per_lane);
    memset(mDMABuffer3, 0, buffer_size_per_lane);
