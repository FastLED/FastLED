...
// Definition for a single channel clockless controller for the sam family of
// arm chips, like that used in the due and rfduino See clockless.h for detailed
// info on how the template parameters are used.
...
class ClocklessController : public CPixelLEDController<RGB_ORDER> {
  ...
  protected:
    ...
    writeBits(FASTLED_REGISTER uint32_t &next_mark,
              FASTLED_REGISTER data_ptr_t port, FASTLED_REGISTER uint8_t &b) {
        // Make sure we don't slot into a wrapping spot, this will delay up
        // to 12.5µs for WS2812 bool bShift=0; while(VAL < (TOTAL*10)) {
        // bShift=true; } if(bShift) { next_mark = (VAL-TOTAL); };
        ...
        for (FASTLED_REGISTER uint32_t i = BITS; i > 0; --i) {
            ...
            *port = 1;
            ...
            // how long we want to wait next depends on whether or not our bit
            // is set to 1 or 0
            if (b & 0x80) {
                ...
                // we're a 0, wait until there's less than (T2+T3+slop) clocks
                // left in this bit
                while ((next_mark - DUE_TIMER_VAL) >
                       (T2 + T3 + 6 + TADJUST + TADJUST))
                    ;
            }
            *port = 0;
            b <<= 1;
...
#define FORCE_REFERENCE(var) asm volatile("" : : "r"(var))
    ...
    static uint32_t showRGBInternal(PixelController<RGB_ORDER> pixels) {
        ...
        FASTLED_REGISTER data_ptr_t port asm("r7") =
            FastPinBB<DATA_PIN>::port();
        FORCE_REFERENCE(port);
        *port = 0;
