...
// A nop/stub class, mostly to show the SPI methods that are needed/used by the
// various SPI chipset implementations.  Should be used as a definition for the
// set of methods that the spi implementation classes should use (since C++
// doesn't support the idea of interfaces - it's possible this could be done
// with virtual classes, need to decide if i want that overhead)
...
class NRF51SPIOutput {
  ...
  public:
    ...
    __attribute__((always_inline)) {
        ...
        // bool oldWait = sWait;
        // sWait = wait;
        // never going to bother with waiting since we're always running the spi
        // clock at max speed on the rfduino
        // TODO: When we set clock rate, implement/fix waiting properly,
        // otherwise the world hangs up
        return false;
    }
