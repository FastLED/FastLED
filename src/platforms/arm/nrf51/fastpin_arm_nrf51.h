...
#define NR_GPIO_ADDR(base, offset)                                             \
    (*(volatile uint32_t *))((uint32_t)(base + offset))
...
#define _FL_DEFPIN(PIN)                                                        \
    template <>                                                                \
    class FastPin<PIN>                                                         \
        : public _ARMPIN<PIN, 1 << PIN, _R(NR_DIRSET), _R(NR_DIRCLR),          \
                         _R(NR_OUTSET), _R(NR_OUTCLR), _R(NR_OUT)> {};
#else
...
#define _FL_DEFPIN(PIN)                                                        \
    template <> class FastPin<PIN> : public _ARMPIN<PIN, 1 << PIN> {};
#endif
