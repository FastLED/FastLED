...
template <uint8_t PIN, uint8_t _BIT, uint32_t _MASK, int _GRP> class _ARMPIN {
  public:
    ...
    inline static void hi() __attribute__((always_inline)) {
        PORT_IOBUS->Group[_GRP].OUTSET.reg = _MASK;
    }
    inline static void lo() __attribute__((always_inline)) {
        PORT_IOBUS->Group[_GRP].OUTCLR.reg = _MASK;
    }
    inline static void set(FASTLED_REGISTER port_t val)
        __attribute__((always_inline)) {
        PORT_IOBUS->Group[_GRP].OUT.reg = val;
    }
    ...
    inline static void toggle() __attribute__((always_inline)) {
        PORT_IOBUS->Group[_GRP].OUTTGL.reg = _MASK;
    }
...
#define _RD32(T)                                                               \
    struct __gen_struct_##T {                                                  \
        static __attribute__((always_inline)) inline volatile PortGroup *r() { \
            return T;                                                          \
        }                                                                      \
    };
...
#define _FL_DEFPIN(PIN, BIT, L)                                                \
    template <> class FastPin<PIN> : public _ARMPIN<PIN, BIT, 1 << BIT, L> {};
