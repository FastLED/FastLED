...
class ClocklessController : public CPixelLEDController<RGB_ORDER> {
    ...
    writeBitsToBuf(int32_t *out_buf, unsigned int bitpos, uint8_t b) {
        ...
        int32_t bitmask_2 = ((1 << (bitcnt_2 + (BITS - 8))) - 1)
  ...
  public:
    virtual void init() {
#if FASTLED_RP2040_CLOCKLESS_PIO
        if (dma_channel != -1)
            return; // maybe init was called twice somehow? not sure if possible
#endif
        ...
        // find an unclaimed PIO state machine and upload the clockless program
        // if possible there's two PIO instances, each with four state machines,
        // so this should usually work out fine
        ...
        for (unsigned int i = 0; i < NUM_PIOS; i++) {
            ...
            if (offset == -1) {
                pio_sm_unclaim(
                    pio, sm); // unclaim the state machine and skip this PIO
        ...
        if (offset == -1)
            return; // couldn't find good pio and sm
        ...
        float div = clock_get_hz(clk_sys) /
                    (pio_clock_multiplier * CLOCKLESS_FREQUENCY);
    ...
    void showRGBInternal(PixelController<RGB_ORDER> pixels) {
        ...
        while (pixels.has(1)) {
            ...
            bitpos +=
                writeBitsToBuf<8 + XTRA0>((int32_t *)(dma_buf), bitpos, b);
    ...
    void showRGBBlocking(PixelController<RGB_ORDER> pixels) {
        ...
        const int portSetOff =
            (uint32_t)&sio_hw->gpio_set - (uint32_t)&sio_hw->gpio_out;
        const int portClrOff =
            (uint32_t)&sio_hw->gpio_clr - (uint32_t)&sio_hw->gpio_out;

        cli();
        showLedData<portSetOff, portClrOff, T1, T2, T3, RGB_ORDER, WAIT_TIME>(
