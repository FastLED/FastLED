...
template <uint PIN, uint32_t _MASK> class _RP2040PIN {
  public:
    ...
    inline static void setOutput() {
        ...
        sio_hw->gpio_oe_set = _MASK;
    }
    inline static void setInput() {
        gpio_set_function(PIN, GPIO_FUNC_SIO);
        sio_hw->gpio_oe_clr = _MASK;
    }

    inline static void hi() __attribute__((always_inline)) {
        sio_hw->gpio_set = _MASK;
    }
    inline static void lo() __attribute__((always_inline)) {
        sio_hw->gpio_clr = _MASK;
    }
...
#define _FL_DEFPIN(PIN)                                                        \
    template <> class FastPin<PIN> : public _RP2040PIN<PIN, 1 << PIN> {};
