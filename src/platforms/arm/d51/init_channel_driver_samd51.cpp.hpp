...
/// `ClocklessController` (clockless_arm_d51.h) is a `SlimBridgeController` whose
/// per-pin `ClocklessSamd51Driver` self-registers through
...
static void addSpiHardwareIfPossible(ChannelManager& manager) {
    ...
    FL_DBG("SAMD51: Found %s SpiHw4 controllers", hw4Controllers.size());
    ...
    FL_DBG("SAMD51: Found %s SpiHw2 controllers", hw2Controllers.size());
    ...
    if (!controllers.empty()) {
        ...
        if (adapter) {
            ...
            FL_DBG("SAMD51: Registered unified SPI driver with %s controllers (priority %s)", controllers.size(), maxPriority);
        } else {
