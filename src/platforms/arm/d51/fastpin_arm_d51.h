...
#define _FL_DEFPIN(PIN, BIT, L)                                                \
    template <> class FastPin<PIN> : public _ARMPIN<PIN, BIT, 1ul << BIT, L> {};
...
_FL_DEFPIN(
    19, 6,
    0); /* A6 is present in variant.h but couldn't find it on the schematic */
// SDA/SCL
_FL_DEFPIN(21, 12, 0);
_FL_DEFPIN(22, 13, 0);
...
_FL_DEFPIN(PIN_SPI_MISO, 0, 1);
_FL_DEFPIN(PIN_SPI_MOSI, 2, 1);
_FL_DEFPIN(PIN_SPI_SCK, 3, 1);
