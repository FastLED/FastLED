...
- `init_channel_driver_samd51.cpp.hpp`: Registers the SERCOM SPI adapter. The clockless driver self-registers via `ChannelManager::registry()` only when a clockless controller is instantiated, so sketches without clockless strips do not link it.
