...
class FlexibleInlineBlockClocklessController
    : public CPixelLEDController<RGB_ORDER, LANES, __FL_T4_MASK> {
...
// For each pin, if we've hit our lane count, break, otherwise set the pin to
// output, store the bit offset in our offset array, add this pin to the write
// mask, and if this pin ends a block sequence, then break out of the switch as
// well
#define _BLOCK_PIN(P)                                                          \
    case P: {                                                                  \
        if (m_nActualLanes == LANES)                                           \
            break;                                                             \
        FastPin<P>::setOutput();                                               \
    ...
    virtual void init() {
        ...
        for (int i = 0; i < m_nActualLanes; ++i) {
            if (m_bitOffsets[i] < m_nLowBit) {
                m_nLowBit = m_bitOffsets[i];
            }
            if (m_bitOffsets[i] > m_nHighBit) {
                m_nHighBit = m_bitOffsets[i];
            }
        }
    ...
    virtual void
    showPixels(PixelController<RGB_ORDER, LANES, __FL_T4_MASK> &pixels) {
        mWait.wait();
    ...
    writeBits(FASTLED_REGISTER uint32_t &next_mark,
              FASTLED_REGISTER _outlines &b,
              PixelController<RGB_ORDER, LANES, __FL_T4_MASK> &pixels) {
        _outlines b2;
        ...
        for (uint32_t i = 8; i > 0;) {
            ...
            *FastPin<FIRST_PIN>::sport() = m_nWriteMask;
            ...
            uint32_t out = (b2.bg[3][i] << 24) | (b2.bg[2][i] << 16) |
                           (b2.bg[1][i] << 8) | b2.bg[0][i];
            ...
            *FastPin<FIRST_PIN>::cport() = out;
            ...
            if (x < m_nActualLanes) {
                b.bytes[m_bitOffsets[x]] =
                    pixels.template loadAndScale<PX>(pixels, x, d, scale);
                ++x;
                if (x < m_nActualLanes) {
                    b.bytes[m_bitOffsets[x]] =
                        pixels.template loadAndScale<PX>(pixels, x, d, scale);
    ...
    uint32_t showRGBInternal(
        PixelController<RGB_ORDER, LANES, __FL_T4_MASK> &allpixels) {
        ...
        uint32_t wait_off =
            _FASTLED_NS_TO_DWT((WAIT_TIME - INTERRUPT_THRESHOLD));
        ...
        while (allpixels.has(1)) {
            ...
            if (ARM_DWT_CYCCNT > next_mark) {
                if ((ARM_DWT_CYCCNT - next_mark) > wait_off) {
                    sei();
...
template <template <uint8_t DATA_PIN, EOrder RGB_ORDER> class CHIPSET,
          uint8_t DATA_PIN, int NUM_LANES, EOrder RGB_ORDER = GRB>
class __FIBCC : public FlexibleInlineBlockClocklessController<
                    NUM_LANES, DATA_PIN, CHIPSET<DATA_PIN, RGB_ORDER>::__T1(),
                    CHIPSET<DATA_PIN, RGB_ORDER>::__T2(),
                    CHIPSET<DATA_PIN, RGB_ORDER>::__T3(), RGB_ORDER,
                    CHIPSET<DATA_PIN, RGB_ORDER>::__XTRA0(),
                    CHIPSET<DATA_PIN, RGB_ORDER>::__FLIP(),
                    CHIPSET<DATA_PIN, RGB_ORDER>::__WAIT_TIME()> {};
