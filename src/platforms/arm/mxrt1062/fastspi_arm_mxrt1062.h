...
template <uint8_t _DATA_PIN, uint8_t _CLOCK_PIN, uint32_t _SPI_CLOCK_RATE,
          SPIClass &_SPIObject, int _SPI_INDEX>
class Teensy4HardwareSPIOutput {
  ...
  public:
    ...
    Teensy4HardwareSPIOutput(Selectable *pSelect) {
        m_pSelect = pSelect;
        m_bitCount = 0;
    }
    ...
    void inline select() __attribute__((always_inline)) {
        ...
        _SPIObject.beginTransaction(
            SPISettings(_SPI_CLOCK_RATE, MSBFIRST, SPI_MODE0));
        if (m_pSelect != NULL) {
            m_pSelect->select();
        }
    }
    ...
    void inline writeByte(uint8_t b) __attribute__((always_inline)) {
        if (m_bitCount == 0) {
            ...
            // There's been a bit of data written, add that to the output as
            // well
            uint32_t outData = (m_bitData << 8) | b;
            uint32_t tcr = port().TCR;
            port().TCR =
                (tcr & 0xfffff000) |
                LPSPI_TCR_FRAMESZ((8 + m_bitCount) - 1); // turn on 9 bit mode
            port().TDR = outData;                        // output 9 bit data.
            while ((port().RSR & LPSPI_RSR_RXEMPTY))
                ; // wait while the RSR fifo is empty...
            port().TCR = (tcr & 0xfffff000) |
                         LPSPI_TCR_FRAMESZ((8) - 1); // turn back on 8 bit mode
            port().RDR;
            m_bitCount = 0;
        }
    }
