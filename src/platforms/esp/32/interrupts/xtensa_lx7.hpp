...
// high-priority interrupts (level 4 and 5) on ESP32-S3 Xtensa cores.
//
...
/*
 * Xtensa LX6/LX7 Interrupt Architecture References:
 *
...
//=============================================================================
// ESP32-S3 INTERRUPT PRIORITY LEVELS
//=============================================================================

/*
 * ESP32-S3 (Xtensa LX7) Interrupt Levels for FastLED:
 *
...
#define FASTLED_ESP32S3_MAX_OFFICIAL_LEVEL 3    // Maximum supported by RMT driver
#define FASTLED_ESP32S3_MAX_CUSTOM_LEVEL   5    // Maximum with custom implementation
#define FASTLED_ESP32S3_RECOMMENDED_LEVEL  3    // Recommended for FastLED
...
void FL_IRAM fastled_esp32s3_level4_handler(void);

// Level 5 C handler - EXPERIMENTAL, called from xt_highint5 assembly shim
void FL_IRAM fastled_esp32s3_level5_handler(void);
 ...
 *   esp_err_t err = fastled_esp32s3_install_level3_interrupt(
 *       ETS_RMT_INTR_SOURCE, nullptr, &rmt_intr_handle);
 */
esp_err_t fastled_esp32s3_install_level3_interrupt(
    int source,
    void *arg,
 ...
 */
esp_err_t fastled_esp32s3_install_level4_interrupt(
    int source,
    void *arg,
    esp_intr_handle_t *handle
);
 ...
 * - Use fastled_esp32s3_rmt_init_official() for standard integration
 *
 ...
 * - Use fastled_esp32s3_rmt_init_custom() for experimental high-priority
 *
 * Reference: FastLED RMT5 Driver Flicker Analysis
 * https://github.com/FastLED/FastLED/issues/1761
 */

// RMT interrupt source mappings for ESP32-S3
#define FASTLED_ESP32S3_RMT_INTR_SOURCE ETS_RMT_INTR_SOURCE

// Initialize RMT with OFFICIAL FastLED driver (level 1-3, RECOMMENDED)
esp_err_t fastled_esp32s3_rmt_init_official(
    int channel,
    int gpio_num,
 ...
 *     call0 fastled_esp32s3_level5_handler
 *
 *     # Restore registers
