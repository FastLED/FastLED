...
inline size_t wave8EncodeSingleLane(
    ...
    if (output.size() < required_size) {
        FL_WARN("wave8EncodeSingleLane: Output buffer too small (need %s bytes, have %s bytes)", required_size, output.size());
        return 0;
    }
...
inline size_t wave8EncodeDualLane(
    ...
    if (lane0.size() != lane1.size()) {
        FL_WARN("wave8EncodeDualLane: Lane sizes mismatch (lane0=%s bytes, lane1=%s bytes)", lane0.size(), lane1.size());
        return 0;
    }

    const size_t required_size = lane0.size() * 16;
    if (output.size() < required_size) {
        FL_WARN("wave8EncodeDualLane: Output buffer too small (need %s bytes, have %s bytes)", required_size, output.size());
        return 0;
    }
...
inline size_t wave8EncodeQuadLane(
    ...
    for (int i = 1; i < 4; i++) {
        if (lanes[i].size() != lane_size) {
            FL_WARN("wave8EncodeQuadLane: Lane size mismatch (lane0=%s bytes, lane%s=%s bytes)", lane_size, i, lanes[i].size());
            return 0;
        }
    }
    ...
    if (output.size() < required_size) {
        FL_WARN("wave8EncodeQuadLane: Output buffer too small (need %s bytes, have %s bytes)", required_size, output.size());
        return 0;
    }
