...
/**
 * @brief Implementation of RMT RX Decoder
 *
 * Converts RMT symbols to bytes using chipset-specific timing thresholds.
 */
class RmtRxDecoderImpl : public RmtRxDecoder {
  public:
    RmtRxDecoderImpl(const ChipsetTimingRx &timing, uint32_t resolution_hz)
        : timing_(timing), resolution_hz_(resolution_hz), error_count_(0),
          ns_per_tick_(0) {
        ...
        FL_DBG("RmtRxDecoder constructed: resolution="
               << resolution_hz_ << "Hz, ns_per_tick=" << ns_per_tick_);
    }

    ~RmtRxDecoderImpl() override = default;

    bool decode(const RmtSymbol *symbols, size_t symbol_count,
                uint8_t *bytes_out, size_t *bytes_decoded) override {
        ...
        *bytes_decoded = 0;
        ...
        const auto *rmt_symbols =
            reinterpret_cast<const rmt_symbol_word_t *>(symbols);
        ...
        for (size_t i = 0; i < symbol_count; i++) {
            ...
            if (bit < 0) {
                error_count_++;
                FL_DBG("RmtRxDecoder: invalid symbol at index "
        ...
        if (!success) {
            FL_WARN("RmtRxDecoder: high error rate: "
                    << error_count_ << "/" << symbol_count << " symbols ("
    ...
    bool isResetPulse(RmtSymbol symbol) const override {
        // Cast RmtSymbol to rmt_symbol_word_t to access bitfields
        const auto &rmt_sym =
            reinterpret_cast<const rmt_symbol_word_t &>(symbol);
    ...
    int decodeBit(RmtSymbol symbol) const override {
        // Cast RmtSymbol to rmt_symbol_word_t to access bitfields
        const auto &rmt_sym =
            reinterpret_cast<const rmt_symbol_word_t &>(symbol);
        ...
        bool t0h_match =
            (high_ns >= timing_.t0h_min_ns) && (high_ns <= timing_.t0h_max_ns);
        bool t0l_match =
            (low_ns >= timing_.t0l_min_ns) && (low_ns <= timing_.t0l_max_ns);
        ...
        bool t1h_match =
            (high_ns >= timing_.t1h_min_ns) && (high_ns <= timing_.t1h_max_ns);
        bool t1l_match =
            (low_ns >= timing_.t1l_min_ns) && (low_ns <= timing_.t1l_max_ns);
    ...
    size_t getErrorCount() const override { return error_count_; }

    void clear() override {
        error_count_ = 0;
        FL_DBG("RmtRxDecoder: state cleared");
    }
...
fl::shared_ptr<RmtRxDecoder> RmtRxDecoder::create(const ChipsetTimingRx &timing,
                                                  uint32_t resolution_hz) {
    return fl::make_shared<RmtRxDecoderImpl>(timing, resolution_hz);
}
