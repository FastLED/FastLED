...
FL_NO_INLINE FL_COLD void emitAllocTxFallbackLog(
    ...
    case AllocTxFallbackLog::ATTEMPT:
        FL_LOG_RMT("RMT TX allocation failed with %s× buffering (%s words)", mem_blocks, words_needed);
        FL_LOG_RMT("  Attempting fallback to 1× buffering (%s words)...", fallback_words);
        break;
    case AllocTxFallbackLog::SUCCESS:
        FL_LOG_RMT("  [OK] Fallback successful: allocated %s words (single-buffer mode)", fallback_words);
        FL_LOG_RMT("RMT TX channel %s allocated (non-DMA, %s words, single-buffer)", static_cast<int>(channel_id), fallback_words);
        break;
...
FL_NO_INLINE FL_COLD void emitAllocTxFailureDiagnostic(
    ...
    FL_WARN("RMT TX allocation failed for channel %s", static_cast<int>(channel_id));
    FL_WARN("  Requested: %s words (%s× buffer%s)", words_needed, mem_blocks, (networkActive ? ", Network mode" : ""));
    FL_WARN("  Available: %s words", available);
    FL_WARN("  Memory breakdown: Total=%s, Allocated=%s, Reserved=%s (external RMT usage)", total, allocated, reserved);

    if (reserved > 0) {
        FL_WARN("  Suggestion: %s words reserved by external RMT usage (e.g., USB CDC)", reserved);
        FL_WARN("              Consider using DMA channels (use_dma=true) to bypass on-chip memory");
    }
    if (allocated > 0) {
        FL_WARN("  Suggestion: %s words already allocated to other channels", allocated);
        FL_WARN("              Consider reducing LED count or using fewer channels");
    }
    if (mem_blocks > 2 && networkActive) {
        FL_WARN("  Suggestion: Network mode uses 3× buffering (%s words per channel)", (mem_blocks * SOC_RMT_MEM_WORDS_PER_CHANNEL));
...
void RmtMemoryManager::initPlatformLimits(size_t& total_tx, size_t& total_rx) FL_NO_EXCEPT {
    ...
    FL_DBG("RMT Memory (ESP32): %s words GLOBAL POOL (shared TX/RX)", total_tx);
    ...
    total_rx = 0;  // Not used for global pool
    FL_DBG("RMT Memory (ESP32-S2): %s words GLOBAL POOL (shared TX/RX)", total_tx);
    ...
    FL_DBG("RMT Memory (ESP32-S3): TX=%s words, RX=%s words (DEDICATED pools)", total_tx, total_rx);
    ...
    FL_DBG("RMT Memory (ESP32-C3/C6/H2): TX=%s words, RX=%s words (DEDICATED pools)", total_tx, total_rx);
    ...
    FL_WARN("RMT Memory (Unknown platform): TX=%s words, RX=%s words (assumed DEDICATED)", total_tx, total_rx);
#endif
}
...
RmtMemoryManager::RmtMemoryManager(size_t total_tx, size_t total_rx, bool is_global) FL_NO_EXCEPT
    ...
    if (is_global) {
        ...
        FL_DBG("RMT Memory Manager (TEST): %s words GLOBAL POOL", total_tx);
    } else {
        ...
        FL_DBG("RMT Memory Manager (TEST): TX=%s words, RX=%s words (DEDICATED pools)", total_tx, total_rx);
    }
}
...
size_t RmtMemoryManager::getPlatformTxWords() FL_NO_EXCEPT {
    size_t tx_limit = 0, rx_limit = 0;
    initPlatformLimits(tx_limit, rx_limit);
    return tx_limit;
}

size_t RmtMemoryManager::getPlatformRxWords() FL_NO_EXCEPT {
    size_t tx_limit = 0, rx_limit = 0;
    initPlatformLimits(tx_limit, rx_limit);
    return rx_limit;
}

bool RmtMemoryManager::isPlatformGlobalPool() FL_NO_EXCEPT {
#if defined(FL_IS_ESP_32DEV) || defined(FL_IS_ESP_32S2)
...
size_t RmtMemoryManager::calculateMemoryBlocks(bool networkActive) FL_NO_EXCEPT {
    ...
    if (memory_pressure) {
        ...
        FL_LOG_RMT("  Total TX: %s words, Allocated TX: %s words (%s TX channels)", total_memory, allocated_memory, allocated_tx_channels);
        FL_LOG_RMT("  Available: %s words", available_memory);
        FL_LOG_RMT("  Requested: %s blocks (%s words)", requested_blocks, requested_words);
        FL_LOG_RMT("  → At this rate, only %s TX channel(s) would fit", channels_at_requested_rate);
        FL_LOG_RMT("  → Reducing to 1 block (%s words) for better channel density", words_per_block);
        requested_blocks = 1;
    }
    ...
    if (requested_blocks > 2) {
        FL_WARN("Platform limited to 2× buffering (SOC_RMT_TX_CANDIDATES_PER_GROUP=%s), capping from %s to 2 blocks", SOC_RMT_TX_CANDIDATES_PER_GROUP, requested_blocks);
        requested_blocks = 2;
    }
#else
    ...
    if (requested_blocks > max_blocks) {
        FL_WARN("Requested %s blocks exceeds platform max %s (SOC_RMT_TX_CANDIDATES_PER_GROUP), capping to %s", requested_blocks, max_blocks, max_blocks);
...
void RmtMemoryManager::setMemoryBlockStrategy(size_t idleBlocks, size_t networkBlocks) FL_NO_EXCEPT {
    ...
    if (idleBlocks > max_blocks) {
        FL_WARN("RMT setMemoryBlockStrategy: idleBlocks=%s exceeds platform limit=%s, capping", idleBlocks, max_blocks);
        idleBlocks = max_blocks;
    }
    if (networkBlocks > max_blocks) {
        FL_WARN("RMT setMemoryBlockStrategy: networkBlocks=%s exceeds platform limit=%s, capping", networkBlocks, max_blocks);
    ...
    FL_DBG("RMT Memory Strategy updated: idle=%s×, network=%s×", idleBlocks, networkBlocks);
}

void RmtMemoryManager::getMemoryBlockStrategy(size_t& idleBlocks, size_t& networkBlocks) const FL_NO_EXCEPT {
...
result<size_t, RmtMemoryError> RmtMemoryManager::allocateTx(u8 channel_id, bool use_dma, bool networkActive) FL_NO_EXCEPT {
    ...
    if (use_dma) {
        ...
        if (!tryAllocateWords(dma_words, true)) {
            FL_WARN("RMT TX DMA allocation failed for channel %s - insufficient on-chip memory", static_cast<int>(channel_id));
            FL_WARN("  Requested: %s words (1 block for DMA descriptor)", dma_words);
...
result<size_t, RmtMemoryError> RmtMemoryManager::allocateRx(u8 channel_id, size_t symbols, bool use_dma) FL_NO_EXCEPT {
    ...
    if (!tryAllocateWords(words_needed, false)) {
        ...
        FL_WARN("RMT RX allocation failed for channel %s", static_cast<int>(channel_id));
        FL_WARN("  Requested: %s words (%s symbols)", words_needed, symbols);
        FL_WARN("  Available: %s words", available);
        FL_WARN("  Memory breakdown: Total=%s, Allocated=%s, Reserved=%s (external RMT usage)", total, allocated, reserved);

        // Provide actionable suggestions
        if (reserved > 0) {
            FL_WARN("  Suggestion: %s words reserved by external RMT usage", reserved);
            FL_WARN("              Consider using DMA channels (use_dma=true) to bypass on-chip memory");
        }
        if (allocated > 0) {
            FL_WARN("  Suggestion: %s words already allocated to other channels", allocated);
...
void RmtMemoryManager::releaseAllocation(u8 channel_id,
    ...
    FL_LOG_RMT("RMT %s channel %s freed: %s words%s", (is_tx ? "TX" : "RX"), static_cast<int>(channel_id), alloc->words, (alloc->is_dma ? " (DMA)" : ""));
...
void RmtMemoryManager::recordRecoveryAllocation(u8 channel_id, size_t words, bool is_tx) FL_NO_EXCEPT {
    // Check if already exists (shouldn't, but be safe)
    ChannelAllocation* existing = findAllocation(channel_id, is_tx);
    if (existing) {
        FL_WARN("RMT %s channel %s already has allocation during recovery", (is_tx ? "TX" : "RX"), static_cast<int>(channel_id));
        return;
    }
    ...
    FL_LOG_RMT("RMT %s channel %s recovery allocation recorded: %s words", (is_tx ? "TX" : "RX"), static_cast<int>(channel_id), words);
}

size_t RmtMemoryManager::availableTxWords() const FL_NO_EXCEPT {
    return getAvailableWords(true);
}

size_t RmtMemoryManager::availableRxWords() const FL_NO_EXCEPT {
...
size_t RmtMemoryManager::getAllocationCount() const FL_NO_EXCEPT {
...
bool RmtMemoryManager::isDMAAvailable() const FL_NO_EXCEPT {
...
bool RmtMemoryManager::allocateDMA(u8 channel_id, bool is_tx) FL_NO_EXCEPT {
#if FASTLED_RMT5_DMA_SUPPORTED
    if (mDMAAllocation.allocated) {
        FL_WARN("DMA allocation failed: DMA already allocated to %s channel %s", (mDMAAllocation.is_tx ? "TX" : "RX"), static_cast<int>(mDMAAllocation.channel_id));
        return false;
    }
    ...
    FL_LOG_RMT("DMA allocated to %s channel %s | DMA slots: 1/%s", (is_tx ? "TX" : "RX"), static_cast<int>(channel_id), FASTLED_RMT5_MAX_DMA_CHANNELS);
    return true;
#else
    (void)channel_id;
    (void)is_tx;
...
void RmtMemoryManager::freeDMA(u8 channel_id, bool is_tx) FL_NO_EXCEPT {
    ...
    if (mDMAAllocation.channel_id != channel_id || mDMAAllocation.is_tx != is_tx) {
        FL_WARN("DMA free mismatch: expected %s channel %s, got %s channel %s", (mDMAAllocation.is_tx ? "TX" : "RX"), static_cast<int>(mDMAAllocation.channel_id), (is_tx ? "TX" : "RX"), static_cast<int>(channel_id));
        return;
    }

    FL_LOG_RMT("DMA freed from %s channel %s | DMA slots: 0/%s", (is_tx ? "TX" : "RX"), static_cast<int>(channel_id), FASTLED_RMT5_MAX_DMA_CHANNELS);
...
int RmtMemoryManager::getDMAChannelsInUse() const FL_NO_EXCEPT {
#if FASTLED_RMT5_DMA_SUPPORTED
...
RmtMemoryManager::findAllocation(u8 channel_id, bool is_tx) FL_NO_EXCEPT {
...
RmtMemoryManager::findAllocation(u8 channel_id, bool is_tx) const FL_NO_EXCEPT {
    for (const auto& alloc : mLedger.allocations) {
        if (alloc.channel_id == channel_id && alloc.is_tx == is_tx) {
            return &alloc;
        }
...
void RmtMemoryManager::reserveExternalMemory(size_t tx_words, size_t rx_words) FL_NO_EXCEPT {
    mLedger.reserved_tx_words = tx_words;
    mLedger.reserved_rx_words = rx_words;

    if (mLedger.is_global_pool) {
        size_t total_reserved = tx_words + rx_words;
        FL_DBG("RMT External Reservation (GLOBAL pool): %s words (TX:%s + RX:%s)", total_reserved, tx_words, rx_words);
        FL_DBG("  Available after reservation: %s/%s words", (mLedger.total_words > total_reserved ? mLedger.total_words - total_reserved : 0), mLedger.total_words);
    } else {
        FL_DBG("RMT External Reservation (DEDICATED pools):");
        FL_DBG("  TX: %s words reserved, %s/%s available", tx_words, (mLedger.total_tx_words > tx_words ? mLedger.total_tx_words - tx_words : 0), mLedger.total_tx_words);
        FL_DBG("  RX: %s words reserved, %s/%s available", rx_words, (mLedger.total_rx_words > rx_words ? mLedger.total_rx_words - rx_words : 0), mLedger.total_rx_words);
    }
}

void RmtMemoryManager::getReservedMemory(size_t& tx_words, size_t& rx_words) const FL_NO_EXCEPT {
