  ...
  if (!FASTLED_RMT_BUILTIN_DRIVER) {
    ...
    if (gRMT_intr_handle == NULL)
      esp_intr_alloc(ETS_RMT_INTR_SOURCE,
                     ...
                     interruptHandler, 0, &gRMT_intr_handle);
  }
...
void IRAM_ATTR ESP32RMTController::doneOnChannel(rmt_channel_t channel,
  ...
  // Inline the code for rmt_set_tx_intr_en(channel, false) and rmt_tx_stop, so
  // it can be placed in IRAM
#if CONFIG_IDF_TARGET_ESP32C3 || CONFIG_IDF_TARGET_ESP32H2
