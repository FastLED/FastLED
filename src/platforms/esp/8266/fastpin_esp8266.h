...
#define _FL_DEFPIN(PIN, REAL_PIN)                                              \
    template <>                                                                \
    class FastPin<PIN> : public _ESPPIN<REAL_PIN, (1 << (REAL_PIN & 0x0F))> {  \
    };
...
// This seems to be the standard Dxx pin mapping on most of the esp boards that
// i've found
_FL_DEFPIN(0, 16);
_FL_DEFPIN(1, 5);
_FL_DEFPIN(2, 4);
