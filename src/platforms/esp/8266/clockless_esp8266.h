...
// Info on reading cycle counter from
...
template <int DATA_PIN, int T1, int T2, int T3, EOrder RGB_ORDER = RGB,
          int XTRA0 = 0, bool FLIP = false, int WAIT_TIME = 85>
...
#define _ESP_ADJ2 (0)
    ...
    writeBits(FASTLED_REGISTER uint32_t &last_mark,
              FASTLED_REGISTER uint32_t b) {
        b <<= 24;
        b = ~b;
        for (FASTLED_REGISTER uint32_t i = BITS; i > 0; --i) {
            ...
            // timing issues here. abort the frame if one bit took to long. if
            // the last of the 24 bits has been sent already, it is too late
    ...
    showRGBInternal(PixelController<RGB_ORDER> pixels) {
        ...
        struct InterruptLock {
            InterruptLock() { os_intr_lock(); }
            ~InterruptLock() { os_intr_unlock(); }
            void Unlock() { os_intr_unlock(); }
            void Lock() { os_intr_lock(); }
        };
...
#if (FASTLED_ALLOW_INTERRUPTS == 1)
                intlock.Lock();
                // if interrupts took longer than 45µs, punt on the current
                // frame
