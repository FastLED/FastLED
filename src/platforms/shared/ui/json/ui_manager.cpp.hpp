    ...
    if (doc.is_object()) {
        ...
        for (auto key : doc.keys()) {
            ...
            if (component) {
                ...
                FL_ERROR("could not find component with ID or name: %s", id_or_name);
            }
        }
    } else {
        // Debug: Show what we actually received instead of just asserting
        fl::string debugJson = doc.to_string();
        FL_WARN("*** UI UPDATE ERROR: Expected JSON object but got %s: %s...", (doc.is_array() ? "array" : "non-object"), debugJson.substr(0, 200).c_str());
