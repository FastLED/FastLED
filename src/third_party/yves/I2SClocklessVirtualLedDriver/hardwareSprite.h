...
class hardwareSprite {
  public:
    hardwareSprite() {
        displaySprite = false;
        leds = (CRGB *)&_spritesleds[_spritenumber * SPRITE_WIDTH *
                                     SPRITE_HEIGHT * nb_components];
    ...
    int offset(int x, int y, int width, int height) {
        ...
        if ((posX + x) >= width or (posX + x) < 0 or (posY + y) >= height or
            (posY + y) < 0) {
                ...
                for (int j = 0; j < SPRITE_HEIGHT; j++) {
                    if (leds[j * SPRITE_WIDTH + i] != transparentColor) {
                        ...
                        if (_offset >= 0 and _offset < width * height)
                            target[_offset] =
                                (uint16_t)(((j * SPRITE_WIDTH + i) +
                                            spritenumber * SPRITE_WIDTH *
                                                SPRITE_HEIGHT) *
                                               nb_components +
                                           1); // if 0 then no print
                        // else
                        //   Serial.printf("%d %d out\n",i,j);
                        // lednumber[j * WIDTH + i] = offset(i, j, width,
                        // height); Serial.printf("%d %d %d\n",i,j,lednumber[j *
                        // WIDTH + i]);
