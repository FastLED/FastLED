...
if USE_CACHE:
    ...
    def create_trampoline_script(compiler_cmd, script_name):
        ...
        # Extract cache configuration variables
        if cache_type == "sccache":
            cache_dir = cache_config.get("SCCACHE_DIR", "")
            cache_size = cache_config.get("SCCACHE_CACHE_SIZE", "2G")
            cache_dir_var = "SCCACHE_DIR"
            cache_size_var = "SCCACHE_CACHE_SIZE"
        else:  # ccache
            cache_dir = cache_config.get("CCACHE_DIR", "")
            cache_size = cache_config.get("CCACHE_MAXSIZE", "2G")
            cache_dir_var = "CCACHE_DIR"
            cache_size_var = "CCACHE_MAXSIZE"
        ...
        if platform.system() == "Windows":
            ...
            script_content += f"set {{cache_dir_var}}={{cache_dir}}\\n"
            script_content += f"set {{cache_size_var}}={{cache_size}}\\n\\n"
            script_content += "REM Execute " + cache_type + " with the compiler and all arguments\\n"
            script_content += f'"{{cache_executable}}" {{compiler_cmd}} %*\\n'
            script_path += ".bat"
        else:
            # Unix shell script
            ...
            script_content += f'export {{cache_dir_var}}="{{cache_dir}}"\\n'
            script_content += f'export {{cache_size_var}}="{{cache_size}}"\\n\\n'
            script_content += "# Execute " + cache_type + " with the compiler and all arguments\\n"
            script_content += f'exec "{{cache_executable}}" {{compiler_cmd}} "$@"\\n'
    ...
    cc_trampoline = create_trampoline_script(cc_cmd, "cc_trampoline")
    cxx_trampoline = create_trampoline_script(cxx_cmd, "cxx_trampoline")
    
    # Apply trampoline scripts to both environments
