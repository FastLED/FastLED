...
class CacheAwareCompiler:
    ...
    def compile_with_cache(
        ...
        # Log cache results
        total_files = len(ino_files) + sum(len(self.compiler.find_cpp_files_for_example(f)) for f in ino_files)
        log_fn(f"[CACHE] {self.stats['files_skipped']}/{total_files} files unchanged, {self.stats['files_compiled']}/{total_files} need compilation")
        ...
        if files_to_compile or cpp_files_to_compile:
            result = self._run_actual_compilation(
                files_to_compile, cpp_files_to_compile, pch_compatible_files, 
                log_fn, full_compilation, verbose
            )
        else:
        ...
        if total_files > 0:
            ...
            log_fn(f"[CACHE] {hit_rate:.1f}% cache hit rate, {skip_rate:.1f}% files skipped")
    ...
    def _run_actual_compilation(self, ino_files, cpp_files_to_compile, pch_compatible_files, log_fn, full_compilation, verbose):
        ...
        try:
            return compile_examples_simple(
                self.compiler, ino_files, pch_compatible_files, 
                log_fn, full_compilation, verbose
            )
        finally:
            self.compiler.find_cpp_files_for_example = original_method
    
    def _create_success_result(self, file_count):
...
def create_cache_compiler(compiler: Compiler, cache_file: Path = None, verbose: bool = False) -> CacheAwareCompiler:
