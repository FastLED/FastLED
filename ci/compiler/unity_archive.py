...
def build_unity_chunks_and_archive(
    ...
    unity_cpp_paths: List[Path] = [unity_dir / f"unity{i+1}.cpp" for i in range(len(partitions))]
    ...
    archive_result = compiler.create_archive(unity_obj_paths, output_archive, LibarchiveOptions()).result()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Chunked UNITY build and archive for FastLED src/**")
    parser.add_argument("--chunks", type=int, default=1, help="Number of unity chunks (1-4 typical)")
    parser.add_argument("--no-parallel", action="store_true", help="Compile unity chunks sequentially")
    parser.add_argument("--output", type=str, default=str(Path(".build/fastled/libfastled.a")), help="Path to output archive libfastled.a")
    parser.add_argument("--unity-dir", type=str, default=str(Path(".build/fastled/unity")), help="Directory to place generated unity{N}.cpp/.o files")
    parser.add_argument("--use-pch", action="store_true", help="Enable PCH (not recommended for unity builds)")
    ...
    try:
        compiler = create_fastled_compiler(use_pch=args.use_pch, parallel=not args.no_parallel)
