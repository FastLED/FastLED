...
class ProjectManager:
    ...
    def get_project_directory(
        self, board: Board, build_dir: Optional[str] = None
    ) -> Path:
    ...
    def initialize_project(
        ...
        if result.returncode != 0:
            return (
                False,
                f"Failed to init project for {board.board_name}: {result.stderr}",
            )
    ...
    def _configure_platformio_ini(
        self, project_dir: Path, board: Board, defines: Optional[list[str]] = None
    ) -> Tuple[bool, str]:
...
""")
  ...
  "platforms": "*"
    ...
    def _run_initial_build(self, project_dir: Path, board: Board) -> Tuple[bool, str]:
        ...
        locked_print(
            f"Running initial build for {board.board_name} to establish dependencies"
        )
...
""")
        ...
        cmd = [
            ...
            str(project_dir),
            "--environment",
            board.get_real_board_name(),  # Specify environment
        ]
        ...
        if result.returncode == 0:
            ...
            return (
                False,
                f"Initial build failed for {board.board_name}: {result.stderr}",
            )

    def prepare_example_source(
        self, example_path: Path, project_dir: Path
    ) -> Tuple[bool, str]:
    ...
    def clean_project(
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return (
                    False,
                    f"Failed to clean project for {board.board_name}: {result.stderr}",
                )
