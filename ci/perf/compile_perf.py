...
class CompilePerfAnalyzer:
    ...
    def get_phase_times(self) -> Dict[str, float]:
        ...
        preprocess_events = [e for e in self.events if 'Preprocess' in e.get('name', '')]
        result['Preprocessing'] = sum(e.get('dur', 0) for e in preprocess_events) / 1000.0
    ...
    def get_header_times(self, max_depth: int = 2) -> Dict[str, Any]:
        ...
        for event in sorted_sources:
            ...
            if parent:
                ...
                if parent_name in headers and header_name not in headers[parent_name]['children']:
    ...
    def get_top_operations(self, n: int = 20) -> List[Tuple[str, str, float]]:
        ...
        for event in self.events:
            ...
            if any(keyword in name for keyword in ['Parse', 'Instantiate', 'Source', 'Template']):
                if name == 'Source':
                    ...
                    if child_name in headers:
                        ...
                        for grandchild_name in child_info['children']:
                            if grandchild_name in headers:
                                ...
                                tree[name]['children'][child_name]['children'][grandchild_name] = {
                                    'time': grandchild_info['time']
                                }

        return tree

    def generate_report(self, format_type: str = 'text', threshold_ms: float = 50.0) -> str:
        ...
        if total_time > 0:
            lines.append(f"Frontend Time:             {frontend_time:,.2f} ms ({frontend_time/total_time*100:.1f}%)")
            lines.append(f"Backend Time:              {backend_time:,.2f} ms ({backend_time/total_time*100:.1f}%)")
        lines.append("")
        ...
        for phase_name in ['Source', 'Templates', 'Lexing', 'Preprocessing']:
            ...
            if phase_time > 0 and total_time > 0:
                lines.append(f"{phase_name:25s} {phase_time:8.2f} ms ({phase_time/total_time*100:5.1f}%)")
        lines.append("")
        ...
        for root_name, root_info in sorted_tree[:10]:
            if root_info['time'] > 5.0:  # Only show significant headers
                ...
                children = sorted(root_info['children'].items(), key=lambda x: x[1]['time'], reverse=True)
                for i, (child_name, child_info) in enumerate(children[:5]):
                    is_last = i == len(children[:5]) - 1
                    prefix = "  └─" if is_last else "  ├─"
                    lines.append(f"{prefix} {child_info['time']:6.1f} ms - {child_name}")
        ...
        slow_headers = [(name, time) for name, time in fastled_headers if time > threshold_ms]
        if slow_headers:
        ...
        if total_time > 0 and template_time / total_time < 0.15:
            ...
            lines.append(f"⚠️  Template instantiation time is high ({template_time/total_time*100:.1f}% of total)")
    ...
    def generate_json(self) -> Dict[str, Any]:
        ...
        template_time = phases.get('Templates', 0)
        if total_time > 0 and template_time / total_time > 0.15:
            warnings.append(f"Template instantiation time is high ({template_time/total_time*100:.1f}%)")
        ...
        return {
            ...
            'top_headers': [
                {'name': name, 'time': time}
                for name, time in fastled_headers[:20]
            ],
            'top_operations': [
...
def compile_with_trace(source_file: Path, compiler: str = 'clang++', include_dir: Optional[Path] = None) -> Path:
    """
...
def check_thresholds(analyzer: CompilePerfAnalyzer, thresholds_file: Path) -> bool:
    ...
    if total_time > max_total:
        errors.append(f"Total compile time {total_time:.1f}ms exceeds threshold {max_total}ms")
    ...
    for name, time_ms in fastled_headers:
        ...
        if name in known_slow:
            ...
            if time_ms > specific_threshold:
                warnings.append(f"Known slow header {name} ({time_ms:.1f}ms) exceeds its threshold ({specific_threshold}ms)")
            continue

        if time_ms > error_threshold:
            errors.append(f"Header {name} ({time_ms:.1f}ms) exceeds error threshold ({error_threshold}ms)")
        elif time_ms > warning_threshold:
            warnings.append(f"Header {name} ({time_ms:.1f}ms) exceeds warning threshold ({warning_threshold}ms)")
    ...
    if total_time > 0 and (template_time / total_time * 100) > max_template_pct:
        warnings.append(f"Template instantiation time ({template_time/total_time*100:.1f}%) exceeds {max_template_pct}%")
...
def main() -> None:
    ...
    trace_file = compile_with_trace(source_file, compiler, include_dir=project_root / 'src')
