#!/usr/bin/env python3
"""
Python-based fake compiler system for sccache integration.

This module provides Python scripts that act as fake compilers, wrapping the real
...
def create_fake_compiler_script(
    compiler_name: str,
...
This script acts as a fake compiler that wraps the real {compiler_name}
...
def debug_print(msg: str) -> None:
    ...
    if DEBUG:
        print(f"FAKE_COMPILER[{compiler_name}]: {{msg}}", file=sys.stderr)

def main() -> int:
    """Main entry point for fake compiler."""
    try:
        ...
        print(f"ERROR in fake compiler {{compiler_name}}: {{e}}", file=sys.stderr)
        return 1
