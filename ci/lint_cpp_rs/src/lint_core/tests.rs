...
";
    ...
    // Split out to keep every Rust linter source under the 1,000-line
    // ceiling enforced by `rust_lint_source_files_stay_under_one_thousand_lines`.
    // Included inside `mod tests` so both halves share the helpers above.
    include!("tests_checkers.rs");
}
