...
from ci.ci.fingerprint_cache import FingerprintCache, CacheEntry, has_changed
...
class TestFingerprintCache(TestCase):
    ...
    def create_test_file(self, path: Path, content: str, modtime: float = None) -> Path:
    ...
    def test_cache_hit_modtime_unchanged(self) -> None:
        ...
        self.assertLess(elapsed, 0.001, f"Cache hit should be <1ms, took {elapsed*1000:.2f}ms")
    ...
    def test_cache_hit_with_existing_cache(self) -> None:
        ...
        self.assertFalse(result, "File with current modtime in cache should return False")
    ...
    def test_modtime_changed_content_same(self) -> None:
        ...
        self.assertNotEqual(new_modtime, original_modtime, "Touch should change modtime")
        
        # Verify content is still the same
        with open(test_file, 'r') as f:
            current_content = f.read()
        self.assertEqual(current_content, original_content, "Content should be unchanged")
        
        # Test the critical behavior: should return False despite modtime change
        result = cache.has_changed(test_file, original_modtime)
        self.assertFalse(result, "File with changed modtime but same content should return False")
    ...
    def test_file_copy_different_timestamp(self) -> None:
        ...
        result = cache.has_changed(test_file, original_modtime)
        self.assertFalse(result, "File with different timestamp but same content should return False")
    ...
    def test_performance_cache_hit(self) -> None:
        ...
        self.assertLess(avg_time, 1.0, f"Cache hit average {avg_time:.2f}ms should be <1.0ms")
    ...
    def test_performance_large_files(self) -> None:
        ...
        self.assertLess(elapsed, 0.5, f"Large file processing {elapsed*1000:.2f}ms should be <500ms")
    ...
    def test_build_system_workflow(self) -> None:
        ...
        self.assertEqual(len(changed_files), 3, "All new files should be detected as changed")
        ...
        self.assertEqual(len(changed_files), 0, "Unchanged files should not be detected as changed")
        ...
        self.assertEqual(changed_files[0], source_files[0], "Should detect the correct modified file")
