...
def test_ci_platforms_wasm_selects_only_the_wasm_tests() -> None:
    wasm = set(expand("ci-platforms-wasm", LABELS))
    assert wasm == set(expand("ci-test:wasm", LABELS))
    assert wasm and all(cell.startswith("build_wasm") for cell in wasm)
    selected = select("pull_request", pr_event(["ci-platforms-wasm"]), LABELS)
    assert set(selected.cells) == wasm


def test_ci_platforms_unit_test_is_host_only() -> None:
    # A known host-mode label is accepted but selects no board/QEMU/bloat/WASM cell.
    selected = select("pull_request", pr_event(["ci-platforms-unit-test"]), LABELS)
    assert selected.mode == "minimal"
    assert selected.cells == []
    for path in (ROOT / ".github" / "workflows").glob("*.yml"):
        text = path.read_text(encoding="utf-8")
        if "Generated by bash ci-labels sync" in text:
            assert "ci-platforms-unit-test" not in text, path.name


def test_unknown_ci_platforms_label_fails() -> None:
    with pytest.raises(ValueError, match="unknown CI labels"):
        select("pull_request", pr_event(["ci-platforms-typo"]), LABELS)
