...
def demo_basic_usage() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        changed = cache.has_changed(source_file, current_modtime)
        print(f"Third check (touched file): {changed} (expected: False - content unchanged)")
...
def demo_build_system_workflow() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"Files to rebuild: {changed_files} ({len(changed_files)}/{len(source_files)})")
...
def demo_performance() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"Cache miss time: {miss_time*1000:.1f}ms ({miss_time*1000/num_files:.2f}ms per file)")
        ...
        print(f"Cache hit time: {hit_time*1000:.1f}ms ({hit_time*1000/num_files:.2f}ms per file)")
