...
def compile_with_pio_ci(
    ...
    for example_path in example_paths:
        ...
        if has_subdirectories:
            ...
            if verbose:
                locked_print(
                    f"Complex example detected for {example_path.name}, copying all files to preserve directory structure"
                )
            ...
            for src_file in example_path.rglob("*"):
                if src_file.is_file():
                    ...
                    if src_file.name.startswith(".") or src_file.suffix in [
                        ".o",
                        ".a",
                        ".so",
                    ]:
...
"""
            ...
            if not platform:
                ...
                if real_board_name in ["uno", "nano", "leonardo", "micro"]:
                    ...
                    real_board_name.startswith("esp8266") or real_board_name == "esp01"
                ):
                    platform = "espressif8266"
                elif real_board_name.startswith("teensy"):
                    platform = "teensy"
                elif (
                    "stm32" in real_board_name.lower()
                    or "bluepill" in real_board_name.lower()
                ):
                    platform = "ststm32"
                else:
            ...
            if verbose:
                locked_print(
                    f"Created platformio.ini for complex example at {platformio_ini_path}"
                )
            ...
            if verbose:
                locked_print(
                    f"Simple example detected for {example_path.name}, using pio ci"
                )
        ...
        if not has_subdirectories:
            ...
            if board.customsdk:
                cmd_list.extend(
                    ["--project-option", f"custom_sdkconfig={board.customsdk}"]
                )
        else:
            ...
            if (
                all_defines
                or board.board_build_core
                or board.board_build_filesystem_size
                or board.customsdk
            ):
