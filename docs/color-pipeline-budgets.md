...
- **Compiled in but never called costs the whole pipeline.** Linking is decided at compile time, by whether any code path calls `setColorProfile`, not by whether that call runs. `examples/ColorProfile` on ESP32-S3 with the call behind a `volatile` flag that is never set: 586,736 B flash / 84,272 B RAM, against 586,716 / 84,272 bound and 576,268 / 84,264 without the call. That is +10,468 B flash and +8 B static RAM for a profile that is never bound, against +10,448 B / +8 B for binding one: 20 B more, which is the `volatile` guard, so never binding saves nothing. All three are `bash compile` image sizes, the same method as the table above, but measured after #4472 on 2026-09-19; the table's +10,708 B / +536 B is from an earlier master. Before #4472 a compiled-in profile setter also linked the power limiter's estimator and its two 256-entry tables (+1,268 B flash, +536 B RAM) whether or not power limiting was used; the estimator is now passed into the pipeline's power hook by the limiter, so it links only when both are used. All ~100 pipeline symbols link. At runtime an unbound channel takes the legacy path: the hooks are installed only when `setColorProfile` runs, so each frame costs a null check per channel and nothing per pixel. To pay nothing in flash, exclude the call at compile time, as `FASTLED_EXAMPLE_NO_COLOR_PROFILE` does in the example.
