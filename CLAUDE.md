...
- **Missing identity ⇒ push it to FastLED/boards immediately.** We own that repo; pushing is the standard way to get support, not an escalation. Add a curated record on the `other` data branch, run the `Build site` workflow, done — minutes, not a ticket. Do NOT file an issue and wait, and do NOT add a local literal meanwhile. fbuild fetches the catalogue at runtime, so a data-only change needs no fbuild release; bump the `fbuild==X.Y.Z` pin only when ingestion or resolution *logic* changes.
