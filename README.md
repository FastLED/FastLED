...
This is a library for easily & efficiently controlling a wide variety of LED chipsets, like the ones sold by adafruit (Neopixel, DotStar, LPD8806), Sparkfun (WS2801), and aliexpress.  In addition to writing to the
leds, this library also includes a number of functions for high-performing 8bit math for manipulating your RGB values, as well as low level classes for abstracting out access to pins and SPI hardware, while still keeping things as fast as possible.  Tested with Arduino up to 1.6.5 from arduino.cc.
...
Here's a list of all the LED chipsets that are supported.  More details on the led chipsets are included *TODO: Link to wiki page*
...
* Teensy 2, Teensy++ 2, Teensy 3.0, Teensy 3.1/3.2, Teensy LC - arduino compatible from pjrc.com with some extra goodies (note the teensy 3, 3.1, and LC are ARM, not AVR!)
