...
class Args:
    ...
    def parse_args() -> "Args":
        ...
        parser.add_argument("--example", default="Blur", 
                       help="Example to build (default: Blur)")
        parser.add_argument("--verbose", action="store_true",
                       help="Enable verbose output")
        parser.add_argument("--platform", default="uno",
                       help="Platform to build for (default: uno)")
...
def init_platformio_build(args: Args) -> BuildResult:
    ...
    if not ok_copy_src:
        ...
        return BuildResult(
            success=False, output=f"Example not found: {project_root / 'examples' / example}", 
            build_dir=build_dir)
    ...
    if not ok_copy_fastled:
        ...
        return BuildResult(success=False, output=f"Failed to copy FastLED library", build_dir=build_dir)
...
def run_platform_build(args: Args) -> BuildResult:
    ...
    run_cmd: list[str] = [
        "pio", "run", "--project-dir", str(build_dir)
    ]
    if verbose:
    ...
    if result.returncode != 0:
        warnings.warn(f"Build failed: {result.stderr}")
        return BuildResult(success=False, output=result.stderr, build_dir=build_dir)

    return BuildResult(success=True, output=result.stdout, build_dir=build_dir)
