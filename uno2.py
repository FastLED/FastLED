...
class PlatformIoTester:
    def __init__(self, args: Args):
        self.args = args
        self.project_init_result: BuildResult | None = None

    def init_build(self) -> BuildResult:
        if self.project_init_result is None:
            self.project_init_result = init_platformio_build(self.args)
        return self.project_init_result

    def run(self) -> BuildResult:
        build_result = self.init_build()
        if not build_result.success:
            warnings.warn(f"Build failed: {build_result.output}")
            return build_result
        return _run_build(build_result.build_dir, self.args.verbose)
...
def main() -> int:
    ...
    tester = PlatformIoTester(args)
    b: BuildResult = tester.init_build()
    if not b.success:
        warnings.warn(f"Build failed: {b.output}")
        return 1
    b = tester.run()
    if not b.success:
        warnings.warn(f"Build failed: {b.output}")
    ...
    print(f"Build successful: {b.output}")
    return 0
