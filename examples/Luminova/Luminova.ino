...
void resetParticle(P& p, uint32_t tt) {
  // Original: x=360,y=360 (center), a=t*1.25 + noise(I)*W, f=t%2*2-1, g=I, s=5
  int I = (int)(tt / 50);
  p.x = (WIDTH - 1) / 2.0f;
  p.y = (HEIGHT - 1) / 2.0f;
...
void loop() {
  ...
  blur2d(leds, WIDTH, HEIGHT, 24);        // slightly gentler blur for 32x32
  ...
  for (int i = 0; i < MAXP; ++i) {
    ...
    p.s *= 0.997f;
    if (p.s < 0.5f) { p.alive = false; continue; } // cheap cull
 ...
 * - Want color instead of white? Replace plotDot/plotSoftDot to use CHSV with hue based on p.g or p.a.
