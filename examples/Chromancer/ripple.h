  ...
  private:
    ...
    */
    ...
    void renderLed(byte ledColors[40][14][3], unsigned long age) {
      ...
      ledColors[position[0]][position[1]][0] = byte(fl::fl_min(255, fl::fl_max(0, int(fmap(float(age), 0.0, float(lifespan), (color >> 8) & 0xFF, 0.0)) + red)));
      ledColors[position[0]][position[1]][1] = byte(fl::fl_min(255, fl::fl_max(0, int(fmap(float(age), 0.0, float(lifespan), (color >> 16) & 0xFF, 0.0)) + green)));
      ledColors[position[0]][position[1]][2] = byte(fl::fl_min(255, fl::fl_max(0, int(fmap(float(age), 0.0, float(lifespan), color & 0xFF, 0.0)) + blue)));
