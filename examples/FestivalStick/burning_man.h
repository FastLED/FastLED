...
enum {
  kButtonNo,      // No button activity
  kButtonShort,   // Short press detected
  kButtonLong,    // Long press detected
};
...
void setup() {
    ...
    // Set power limiter to 1000 milliamps at 5V
...
bool button_clicked() {
    ...
    static u32 click_delay = 50;
    if (now - last_click_time < click_delay) {
        return false;
    }
    last_click_time = now;
    static bool last_button_state = false;
    bool current_button_state = button_pressed();
    bool clicked = false;
...
// Query UI state function that differentiates between short and long button presses
int queryUiState() {
    static bool last_button_state = false;
    static u32 press_start_time = 0;
    ...
    // Button is being held
        ...
        if (!press_processed && (now - press_start_time) >= LONG_PRESS_DURATION) {
    ...
    // Button just released
        ...
        if (!press_processed && (now - press_start_time) >= SHORT_PRESS_DURATION) {
            press_processed = true;
            last_button_state = current_button_state;
            return kButtonShort;
        }
        press_processed = false; // Reset for next press
    }

    last_button_state = current_button_state;
    return kButtonNo; // No button activity
}
...
void fillFrameBufferFire(uint32_t now) {
    ...
    for (int w = 0; w < width; w++) {
        for (int h = 0; h < height; h++) {
            ...
            CRGB color = ColorFromPalette(myPal, palette_index, 32);
