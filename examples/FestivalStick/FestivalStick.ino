...
fl::vector<vec3f> makeCorkScrew(int num_leds, float leds_per_turn, float width_cm) {
    ...
    const float height_per_led = height_per_turn_cm / leds_per_turn; // this is the changing height per led.
    const float total_height = height_per_turn_cm * total_turns; // total height of the corkscrew
    fl::vector<vec3f> out;
...
fl::vector<vec3f> mapCorkScrew = makeCorkScrew(NUM_LEDS, 24.0f, 50.0f);
...
void setup() {
    ...
    for (auto it : mapCorkScrew) {
        // Initialize all LEDs to black
        FASTLED_WARN("LED Position" << it.x << "," << it.y << "," << it.z);
    }
    FASTLED_WARN("DONE");
}
