/// @file    Esp32C3_SingleSPI_ISR.ino
/// @brief   ESP32-C3/C2 Single-SPI (1-way) ISR driver test for baseline validation
/// @example Esp32C3_SingleSPI_ISR.ino
///
/// This example demonstrates the 1-way (single pin) SPI ISR driver, which uses:
/// - 1 data pin (D0) for serial bit output
/// - 1 clock pin for synchronization
/// - High-priority ISR for minimal jitter
///
/// The 1-way architecture is the simplest variant, ideal for:
/// - Baseline testing and validation
/// - Debugging ISR behavior
/// - Understanding the core ISR engine logic
...
void setup() {
    ...
    Serial.println("Esp32C3_SingleSPI_ISR setup starting");
    Serial.println("1-way (Single-pin) SPI ISR driver test");
    Serial.println("Architecture: 1 data pin + 1 clock pin");
...
void loop() {
    ...
    if (iteration >= NUM_TEST_ITERATIONS) {
        ...
        if (!testComplete) {
            ...
            Serial.println("✓ Esp32C3_SingleSPI_ISR test PASSED");
            testComplete = true;
        }
        delay(5000);
        return;
    ...
    for (uint16_t i = 0; i < printCount; i++) {
        ...
        switch (events[i].type()) {
            ...
            case GPIOEventType::SetBits:
                ...
                Serial.print(" (bit: ");
                Serial.print(events[i].payload.gpio_mask & 1);
                Serial.println(")");
                break;
            case GPIOEventType::ClearBits:
